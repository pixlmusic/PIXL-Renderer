...
[numthreads(NUMTHREAD_X, NUMTHREAD_Y, NUMTHREAD_Z)] void main(
	...
	uint2 contactLightIndices = uint2(0xffffffffu, 0xffffffffu);
	float2 contactLightScores = float2(-1.0f, -1.0f);
	...
	float3 clusterCenter = (cluster.minPoint.xyz + cluster.maxPoint.xyz) * 0.5f;
	...
	for (uint i = 0; i < LightCount; i++) {
		...
		[branch] if (LightIntersectsCluster(positionVS, radiusSquared, cluster))
		{
			...
			// Select once per cluster instead of rescanning every light in every
			// material pixel. The exact per-pixel pass later compares these two
			// candidates against all strict lights.
			float3 toLight = positionVS - clusterCenter;
			float luminance = dot(max(light.color, 0.0f), float3(0.2126f, 0.7152f, 0.0722f));
			float score = luminance * max(light.fade, 0.0f) /
				max(dot(toLight, toLight) + max(light.sizeBias, 1.0f), 1.0f);
			if ((light.lightFlags & LightFlags::Disabled) != 0)
				score = -1.0f;
			if (score > contactLightScores.x) {
				contactLightScores.y = contactLightScores.x;
				contactLightIndices.y = contactLightIndices.x;
				contactLightScores.x = score;
				contactLightIndices.x = i;
			} else if (score > contactLightScores.y) {
				contactLightScores.y = score;
				contactLightIndices.y = i;
			}
	...
	// Re-evaluate intersection when writing. This second bounded light walk is
	// substantially cheaper than spilling a 256-entry private array for every
	// one of the 1024 cluster threads in a group.
	uint writeIndex = 0;
	for (uint j = 0; j < LightCount && writeIndex < visibleLightCount; ++j) {
		Light light = lights[j];
		float3 positionVS = FrameBuffer::WorldToView(light.positionWS.xyz);
		if (LightIntersectsCluster(positionVS, light.radius * light.radius, cluster)) {
			lightIndexList[offset + writeIndex] = j;
			writeIndex++;
		}
	}

	LightGrid output = {
		offset, visibleLightCount, contactLightIndices.x, contactLightIndices.y
	};
