...
[numthreads(8, 8, 1)] void main(const uint2 dtid : SV_DispatchThreadID) {
	...
	// RELAX/FFX-inspired anti-firefly bounds and variance guidance. The cross
	// footprint is deliberately small: it preserves Skyrim texture detail while
	// keeping high-energy cache samples from bleeding through edges.
	int2 maxCoord = int2(OUT_FRAME_DIM) - 1;
	int2 pL = clamp(int2(dtid) + int2(-1, 0), 0, maxCoord);
	int2 pR = clamp(int2(dtid) + int2(1, 0), 0, maxCoord);
	int2 pU = clamp(int2(dtid) + int2(0, -1), 0, maxCoord);
	int2 pD = clamp(int2(dtid) + int2(0, 1), 0, maxCoord);
	float4 yL = srcIlY[pL];
	float4 yR = srcIlY[pR];
	float4 yU = srcIlY[pU];
	float4 yD = srcIlY[pD];
	float4 localMinY = min(ilY, min(min(yL, yR), min(yU, yD)));
	float4 localMaxY = max(ilY, max(max(yL, yR), max(yU, yD)));
	float meanY = (ilY.x + yL.x + yR.x + yU.x + yD.x) * 0.2;
	float varianceY = ((ilY.x - meanY) * (ilY.x - meanY) +
		(yL.x - meanY) * (yL.x - meanY) + (yR.x - meanY) * (yR.x - meanY) +
		(yU.x - meanY) * (yU.x - meanY) + (yD.x - meanY) * (yD.x - meanY)) * 0.2;
	float sigmaY = sqrt(max(varianceY, 1e-6));
	float varianceEnvelope = 2.5;
#if defined(QUARTER_RES)
	varianceEnvelope = 1.75;
#elif defined(HALF_RES)
	varianceEnvelope = 2.0;
#endif
	localMinY.x = max(localMinY.x, meanY - varianceEnvelope * sigmaY);
	localMaxY.x = min(localMaxY.x, meanY + varianceEnvelope * sigmaY);

	// [Improvement] The luminance (Y) channel got a proper variance-clipped
	// firefly bound above, but the chroma (CoCg) channel was only clamped to
	// the flat min/max of its 4-neighborhood, with no variance guard. A
	// colored firefly (a saturated glint or a strongly-tinted world-cache
	// sample) can still pass that flat clamp untouched and then get smeared
	// across the kernel as a colored blob during the weighted sum below.
	// Apply the same mean/variance clip used for Y to Co and Cg individually
	// so chroma outliers get the same treatment as luminance outliers.
	float2 cL = srcIlCoCg[pL];
	float2 cR = srcIlCoCg[pR];
	float2 cU = srcIlCoCg[pU];
	float2 cD = srcIlCoCg[pD];
	float2 localMinCoCg = min(ilCoCg, min(min(cL, cR), min(cU, cD)));
	float2 localMaxCoCg = max(ilCoCg, max(max(cL, cR), max(cU, cD)));
	float2 meanCoCg = (ilCoCg + cL + cR + cU + cD) * 0.2;
	float2 varianceCoCg = ((ilCoCg - meanCoCg) * (ilCoCg - meanCoCg) +
		(cL - meanCoCg) * (cL - meanCoCg) + (cR - meanCoCg) * (cR - meanCoCg) +
		(cU - meanCoCg) * (cU - meanCoCg) + (cD - meanCoCg) * (cD - meanCoCg)) * 0.2;
	float2 sigmaCoCg = sqrt(max(varianceCoCg, 1e-6));
