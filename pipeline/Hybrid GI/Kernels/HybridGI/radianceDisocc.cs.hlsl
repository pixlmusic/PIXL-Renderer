...
Texture2D<half4> srcPrevBentVisibility : register(t10);   // encoded world bent normal.xy, visibility, confidence
...
RWTexture2D<float4> outRemappedBentVisibility : register(u6);

#if defined(TEMPORAL_DENOISER) || defined(HALF_RATE)
#	define REPROJECTION
#endif

// [Cleanup] Dropped the unused `prev_ambient` inout parameter that was
// threaded through every call site but never read or written anywhere in the
// function body - it just added dead register traffic to every one of the
// four bilinear taps below.
void readHistory(
	...
	inout half4 prev_gi_specular, inout half4 prev_bent_visibility, inout float wsum)
{
	...
	float normal_prod = saturate(dot(curr_normal, prev_normal));
	...
	bool normal_pass = normal_prod >= 1.0 - NormalDisocclusion;
...
[numthreads(8, 8, 1)] void main(const uint2 pixCoord : SV_DispatchThreadID) {
	...
	if (curr_depth < FP_Z) {
		...
		outRemappedPrevGISpecular[pixCoord] = 0.0;
		outRemappedBentVisibility[pixCoord] = 0.0;
		return;
	}
