...
#ifndef TERRAIN_DETAIL_HLSLI
#define TERRAIN_DETAIL_HLSLI

#include "Common/SharedData.hlsli"

#ifndef USE_PIXL_TERRAIN_DETAIL
#	define USE_PIXL_TERRAIN_DETAIL 1
#endif
...
#if USE_PIXL_TERRAIN_DETAIL
	o.offset3 = hash2D2D(c2.cell);
	o.tapWeights = float3(w1, w2, w3) * rcp(max(w1 + w2 + w3, 1e-8));
#endif
	return o;
}
...
inline float4 StochasticEffect(Texture2D tex, SamplerState samp, float2 uv, StochasticOffsets offsets)
{
	...
	#if USE_PIXL_TERRAIN_DETAIL
	float h3 = lerp(dot(s3.rgb, LUMINANCE_WEIGHTS), s3.a, step(0.001, s3.a));
	float3 weights = offsets.tapWeights * (1.0f + HEIGHT_INFLUENCE * float3(h1, h2, h3));
	weights *= rcp(max(dot(weights, 1.0f.xxx), 1e-8f));
...
inline float4 StochasticEffectParallax(Texture2D tex, SamplerState samp, float2 uv, float mipLevel, StochasticOffsets offsets)
{
	...
	#if USE_PIXL_TERRAIN_DETAIL
	float4 s3 = tex.SampleLevel(samp, uv + offsets.offset3, mipLevel);
	float3 weights = offsets.tapWeights * (1.0f + HEIGHT_INFLUENCE * float3(s1.a, s2.a, s3.a));
	weights *= rcp(max(dot(weights, 1.0f.xxx), 1e-8f));
	return s1 * weights.x + s2 * weights.y + s3 * weights.z;
	#else
