...
float GetInterpolatedHeight(float2 pxCoord, bool isVertical)
{
	...
	bool inBoundA = all(lerpPxCoordA >= 0) && all(lerpPxCoordA < int2(dims));
	bool inBoundB = all(lerpPxCoordB >= 0) && all(lerpPxCoordB < int2(dims));
	float heightA = TexHeight[clamp(lerpPxCoordA, 0, int2(dims) - 1)];
	float heightB = TexHeight[clamp(lerpPxCoordB, 0, int2(dims) - 1)];
...
float2 GetInterpolatedHeightRW(float2 pxCoord, bool isVertical)
{
	...
	bool inBoundA = all(lerpPxCoordA >= 0) && all(lerpPxCoordA < int2(dims));
	bool inBoundB = all(lerpPxCoordB >= 0) && all(lerpPxCoordB < int2(dims));
	float2 heightA = RWTexShadowHeights[clamp(lerpPxCoordA, 0, int2(dims) - 1)];
	float2 heightB = RWTexShadowHeights[clamp(lerpPxCoordB, 0, int2(dims) - 1)];
