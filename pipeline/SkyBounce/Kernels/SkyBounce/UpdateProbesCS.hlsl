...
[numthreads(8, 8, 1)] void main(uint3 dtid : SV_DispatchThreadID) {
	...
	bool inFront = cellCentreCS.w > 0.0f;
	float2 screenUV = 0.0f;
	if (inFront)
		screenUV = cellCentreCS.xy * rcp(cellCentreCS.w) * float2(0.5f, -0.5f) + 0.5f;
	bool onScreen = inFront && all(screenUV > 0.0f) && all(screenUV < 1.0f);

	if (onScreen) {
		float shadowSample = 1.0;
		// Only materialize the split distances and the selected projection matrix.
		// Copying the full light struct can otherwise inflate register pressure with
		// two unused inverse matrices and the unused start-split data.
		float2 endSplitDistances = DirectionalShadowLights[0].EndSplitDistances;

		uint bitIndex = SharedData::FrameCountAlwaysActive & 31u;
		...
		if (linearDepth > 0 && linearDepth < endSplitDistances.y) {
			float3 positionWS = jitteredMS + FrameBuffer::CameraPosAdjust.xyz;

			uint cascadeIndex = (linearDepth > endSplitDistances.x) ? 1u : 0u;

			float3 positionLS = mul(DirectionalShadowLights[0].ShadowProj[cascadeIndex], float4(positionWS, 1)).xyz;
			...
			float fade = saturate(linearDepth / endSplitDistances.y);
			// Exact fade^16 polynomial using multiplies instead of a generic pow.
			float fade2 = fade * fade;
			float fade4 = fade2 * fade2;
			float fade8 = fade4 * fade4;
			float fade16 = fade8 * fade8;
			float fadeFactor = 1.0f - fade16;
			shadowSample = lerp(1.0f, shadowSample, fadeFactor);
		}
		...
		float shadow = float(countbits(bitmask)) * (1.0f / 32.0f);
