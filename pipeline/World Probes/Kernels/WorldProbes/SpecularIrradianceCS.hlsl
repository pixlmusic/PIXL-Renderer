...
// Part of specular AmbientProbe split-sum approximation.
...
#ifndef PIXL_SPECULAR_IBL_SAMPLES
#	define PIXL_SPECULAR_IBL_SAMPLES 64
#endif

static const uint NumSamples = PIXL_SPECULAR_IBL_SAMPLES;
...
float3 getSamplingVector(uint3 ThreadID)
{
	...
	// Use texel-centred directions. This keeps the GGX convolution aligned with
	// capture/inference and removes the previous one-sided face-domain bias.
	float2 st = (float2(ThreadID.xy) + 0.5) / float2(outputWidth, outputHeight);
...
[numthreads(8, 8, 1)] void main(uint3 ThreadID : SV_DispatchThreadID) {
	...
	for (uint i = 0; i < NumSamples; ++i) {
		...
		if (cosLi > 0.0) {
			...
			float pdf = max(ndfGGX(cosLh, roughness) * 0.25, 1e-6f);
