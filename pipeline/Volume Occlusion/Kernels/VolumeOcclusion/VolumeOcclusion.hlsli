#ifndef __VOLUME_OCCLUSION_HLSLI__
#define __VOLUME_OCCLUSION_HLSLI__

// Variance Shadow Maps (VSM)
// Chebyshev's inequality on filtered depth moments

namespace VolumeOcclusion
{
	#ifndef USE_PIXL_VOLUME_OCCLUSION
	#	define USE_PIXL_VOLUME_OCCLUSION 1
	#endif
	...
	#if USE_PIXL_VOLUME_OCCLUSION
		// Depth-scaled receiver uncertainty suppresses VSM acne after the shadow
		// map is downsampled while retaining contact close to the light.
		variance += 2.0e-5f * depth * depth;
	#endif
		float d = depth - moments.x;
		float pMax = variance / (variance + d * d);
		float lit = (depth <= moments.x) ? 1.0f : pMax;
	#if USE_PIXL_VOLUME_OCCLUSION
		return smoothstep(0.02f, 1.0f, lit);
	#else
		return lit;
	#endif
	}
...
#endif  // __VOLUME_OCCLUSION_HLSLI__
