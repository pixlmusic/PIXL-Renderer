...
namespace Util
{
	...
	namespace PathHelpers
	{
		...
		/**
		 * Gets the PIXL-owned data root. The SKSE loader DLL remains beside this
		 * directory, while all mutable and packaged renderer data is grouped here.
		 * @return Data / "SKSE" / "Plugins" / "PIXL"
		 */
		std::filesystem::path GetPluginPath();
		/** @brief Imports compatible user-owned state from earlier renderer layouts once. */
		void MigrateLegacyPluginState();
		/** @brief Gets the root used for PIXL capture bundles. */
		std::filesystem::path GetDiagnosticsPath();

		/**
		 * Gets PIXL's private ImGui layout state.
		 */
		std::filesystem::path GetImGuiIniPath();

		/**
		 * Gets the PIXL presentation asset directory.
		 */
		std::filesystem::path GetInterfacePath();

		/**
		 * Gets the PIXLRenderer Fonts directory path
		 * @return Interface / "Fonts"
		 */
		std::filesystem::path GetFontsPath();

		/**
		 * Gets the PIXL visual-asset directory path.
		 * @return Interface / "Visuals"
		 */
		std::filesystem::path GetIconsPath();

		/**
		 * Gets the PIXLRenderer Cursors directory path
		...
		/**
		 * Gets the WaterbodyCache directory path
		 * @return PIXL plugin path / "World" / "Water"
		 */
		...
		/**
		 * Gets the PIXL module catalog where integrated descriptors are stored.
		 * @return Data / "Shaders" / "PIXL" / "Modules"
		 */
		std::filesystem::path GetModuleCatalogPath();

		/**
		 * Gets the deployed descriptor for an integrated render module.
		 * @param moduleId Stable module identifier.
		 */
		std::filesystem::path GetModuleDescriptorPath(const std::string& moduleId);

		/**
		 * Gets the deployed shader-kernel directory for an integrated module.
		 * @param moduleId Stable module identifier.
		 */
		 ...
		 * @return  <mod_root> / "SKSE" / "Plugins" / "PIXLRenderer" / "Themes"
		 */
		std::filesystem::path GetThemesRealPath();

		/**
		 * Returns the physical module-catalog path, bypassing a virtual mod filesystem.
		 */
		 ...
		 * file system. Paths like "Data\Shaders\ImageReconstruction\FidelityFX" don't exist on disk when
		 ...
		 *
		 * @param dataRelativePath Path relative to the game root (e.g. L"Data\\Shaders\\ImageReconstruction\\FidelityFX")
		...
		/**
		 * Returns the path to the PIXL Renderer log file in the default SKSE logging folder.
		 * @return Documents / "My Games" / "Skyrim..." / "SKSE" / "PIXLRenderer.log"
		 */
