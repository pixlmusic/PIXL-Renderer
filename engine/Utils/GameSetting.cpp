...
namespace Util
{
	static constexpr std::string_view CS_SETTINGS_PATH{ "Data/SKSE/Plugins/PIXLRenderer/SkyrimOverwrite.ini"sv };
	...
	void RenderImGuiElement(const std::string& settingName, const GameSetting& settingData, T* valuePtr, const std::string& collectionName)
	{
		...
		if (settingData.offset != 0) {
			...
			if (auto _tt = HoverTooltipWrapper()) {
				const auto copyTooltip = std::format("Copy {} '{}' to clipboard.", collectionName, settingName);
				ImGui::TextUnformatted(copyTooltip.c_str());
			}
		}
