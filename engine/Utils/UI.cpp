...
namespace Util
{
	...
	static void PerformClearShaderCache()
	{
		...
		if (shaderCache) {
			...
			if (shaderCache->IsDiskCache()) {
				shaderCache->DeleteDiskCache("explicit Clear Shader Cache button");
			}
		}
	}
	...
	bool FeatureMatchesSearch(RenderModule* feat, const std::string& searchQuery)
	{
...
#define IM_VK_KEYPAD_ENTER (VK_RETURN + 256)
	...
	bool UIntCheckbox(const char* label, std::uint32_t* value)
	{
		if (!value)
			return false;
		...
		// Canonicalize on every draw so hand-edited JSON and legacy one-byte writes
		// cannot leave a non-zero high byte in a GPU-facing toggle.
		*value = checked ? 1u : 0u;
		return changed;
	}

	bool UIntSlider(const char* label, std::uint32_t* value, int minimum, int maximum,
		const char* format, ImGuiSliderFlags flags)
	{
		...
		const auto clamped = std::clamp<std::uint32_t>(original,
			static_cast<std::uint32_t>(std::max(minimum, 0)),
			static_cast<std::uint32_t>(std::max(maximum, minimum)));
		int editValue = static_cast<int>(clamped);
		const bool changed = ImGui::SliderInt(label, &editValue, minimum, maximum, format, flags);
		*value = static_cast<std::uint32_t>(std::clamp(editValue, minimum, maximum));
		return changed || original != *value;
	}

	namespace WeatherUI
	{
		bool IsWeatherControlled(RenderModule* feature, const char* settingName)
		{
		...
		bool SliderFloat(const char* label, RenderModule* feature, const char* settingName, float* value, float min, float max, const char* format)
		{
			...
			if (isControlled) {
				...
				if (ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled)) {
					...
					ImGui::TextDisabled("PIXL blends this value automatically for the active weather.");
					ImGui::PopTextWrapPos();
					ImGui::EndTooltip();
				}
		...
		bool Checkbox(const char* label, RenderModule* feature, const char* settingName, bool* value)
		{
			bool isControlled = IsWeatherControlled(feature, settingName);
		...
		bool UIntCheckbox(const char* label, RenderModule* feature, const char* settingName, std::uint32_t* value)
		{
			if (!value)
				return false;
			bool checked = *value != 0u;
			const bool changed = Checkbox(label, feature, settingName, &checked);
			if (changed)
				*value = checked ? 1u : 0u;
			return changed;
		}

		bool ColorEdit3(const char* label, RenderModule* feature, const char* settingName, float col[3])
		{
			bool isControlled = IsWeatherControlled(feature, settingName);
		...
		bool ColorEdit4(const char* label, RenderModule* feature, const char* settingName, float col[4])
		{
			bool isControlled = IsWeatherControlled(feature, settingName);
	...
	namespace ConstrainedUI
	{
		namespace
		{
			...
			void RenderConstraintTooltip(const ModuleRules::ConstraintResult& constraint)
			{
				if (!ImGui::IsItemHovered(ImGuiHoveredFlags_AllowWhenDisabled))
					return;
				...
				ImGui::Separator();
				ImGui::Text("Forced value: %s", ModuleRules::FormatConstraintValue(constraint.forcedValue).c_str());
				ImGui::PopTextWrapPos();
				ImGui::EndTooltip();
			}
		}

		bool Checkbox(const char* label, bool* value, const ModuleRules::SettingId& settingId)
		{
			auto constraint = ModuleRules::GetConstraints(settingId);
		...
		bool SliderFloat(const char* label, float* value, float min, float max,
			const ModuleRules::SettingId& settingId, const char* format)
		{
			auto constraint = ModuleRules::GetConstraints(settingId);
