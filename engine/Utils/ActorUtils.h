...
{
	/**
	 * @brief Detailed world-space Havok bound used by GroundResponse.
	 *
	 * Unlike GetShapeBound(), this preserves the live rigid-body AABB and a
	 * stable loaded-body identity.  Existing callers can continue using the
	 * legacy center/radius API unchanged.
	 */
	struct DetailedShapeBound
	{
		RE::NiPoint3 center{};
		...
		RE::hkpShapeType shapeType = RE::hkpShapeType::kInvalid;
	};

	/**
	 * @brief Gets the live world-space AABB for one bhk rigid body.
	 *
	 * Uses hkpShape::GetAabbImpl with the rigid body's current Havok transform,
	 * so capsules, boxes, convex shapes and compound/list shapes all follow the
	 * animated/ragdoll pose without reducing the body to a single actor sphere.
	 */
