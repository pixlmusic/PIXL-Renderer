...
namespace Util
{
	...
	*/
	...
	/** @brief Type-safe ImGui checkbox for 32-bit shader/constant-buffer toggles. */
	bool UIntCheckbox(const char* label, std::uint32_t* value);

	/** @brief Type-safe ImGui integer slider for unsigned shader settings. */
	bool UIntSlider(const char* label, std::uint32_t* value, int minimum, int maximum,
		const char* format = "%d", ImGuiSliderFlags flags = ImGuiSliderFlags_None);
	...
	namespace WeatherUI
	{
		 ...
		 */
		bool SliderFloat(const char* label, RenderModule* feature, const char* settingName, float* value, float min, float max, const char* format = "%.3f");
		...
		/** Type-safe weather-aware checkbox for uint-backed GPU flags. */
	...
	namespace ConstrainedUI
	{
		...
		bool SliderFloat(const char* label, float* value, float min, float max,
			const ModuleRules::SettingId& settingId, const char* format = "%.3f");
		...
		bool SliderInt(const char* label, int* value, int min, int max,
			const ModuleRules::SettingId& settingId, const char* format = "%d");
	}
