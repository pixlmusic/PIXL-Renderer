...
namespace PipelineHealth
{
	...
	{
		std::unordered_map<std::string, RenderModule*> featuresByShortName;
		...
		void Initialize()
		{
			...
			const auto& features = RenderModule::GetModuleList();
		...
		RenderModule* FindFeature(const std::string& shortName)
		{
			Initialize();
	...
	static const std::map<std::string, FeatureIssueInfo> s_obsoleteFeatureData = {
		{ "ComplexParallaxMaterials", { .shortName = "ComplexParallaxMaterials",
										  ...
										  .rejectionReason = "Integrated into MaterialLayers feature",
										  .replacementFeature = "MaterialLayers",
										  .userMessage = "This functionality is now built into PIXL Renderer. Remove the old feature as it's no longer needed.",
		...
		{ "WaterBlending", { .shortName = "WaterBlending",
							   ...
							   .rejectionReason = "Replaced by unified WaterOptics feature",
							   .replacementFeature = "WaterOptics",
							   .userMessage = "Water blending functionality is now part of WaterOptics. Install WaterOptics instead for comprehensive water improvements.",
							   .removedInVersion = { 1, 0, 0 },
							   .modifiedShaderDirectory = true,
							   .issueType = FeatureIssueInfo::IssueType::OBSOLETE } },
		{ "WaterCaustics", { .shortName = "WaterCaustics",
							   ...
							   .rejectionReason = "Replaced by unified WaterOptics feature",
							   .replacementFeature = "WaterOptics",
							   .userMessage = "Water caustics functionality is now part of WaterOptics. Install WaterOptics instead for comprehensive water improvements.",
							   .removedInVersion = { 1, 0, 0 },
							   .modifiedShaderDirectory = true,
							   .issueType = FeatureIssueInfo::IssueType::OBSOLETE } },
		{ "WaterParallax", { .shortName = "WaterParallax",
							   ...
							   .rejectionReason = "Replaced by unified WaterOptics feature",
							   .replacementFeature = "WaterOptics",
							   .userMessage = "Water parallax functionality is now part of WaterOptics. Install WaterOptics instead for comprehensive water improvements.",
							   .removedInVersion = { 1, 0, 0 },
							   .modifiedShaderDirectory = true,
							   .issueType = FeatureIssueInfo::IssueType::OBSOLETE } },
	...
	const std::vector<FeatureIssueInfo>& GetPipelineHealth()
	{
	...
	FeatureFileInfo GetFeatureFileInfo(const std::string& featureName)
	{
		...
		std::filesystem::path deployedIniPath = Util::PathHelpers::GetModuleDescriptorPath(featureName);
		std::filesystem::path deployedShaderDir = Util::PathHelpers::GetModuleKernelPath(featureName);
		...
		const FeatureFileInfo& fileInfo, const std::string& minimumVersionRequired)
	{
		...
		if (!issue.replacementFeature.empty()) {
			RenderModule* replacementFeatureObj = s_featureLookupCache.FindFeature(issue.replacementFeature);
			if (replacementFeatureObj) {
				...
				issue.replacementFeatureModLink = replacementFeatureObj->IsCore() ? "" : replacementFeatureObj->GetModuleSupportLink();
			} else {
			...
			if (issueType == FeatureIssueInfo::IssueType::VERSION_MISMATCH) {
				RenderModule* featureObj = s_featureLookupCache.FindFeature(shortName);
				if (featureObj) {
					...
					issue.replacementFeatureModLink = featureObj->IsCore() ? "" : featureObj->GetModuleSupportLink();
				} else {
	...
	void DrawPipelineHealthUI()
	{
		...
		if (auto section = Util::SectionWrapper(T("menu.issues.unknown_features_header", "Unknown Features"),
				T("menu.issues.unknown_features_desc",
					...
					"They may be from development branches or newer PIXL builds. Since we cannot determine what files they may have modified, "
		...
		if (ImGui::Button(T("menu.issues.open_features_folder", "Open Features Folder"))) {
			std::filesystem::path featuresPath = Util::PathHelpers::GetModuleCatalogRealPath();
		...
		if (!logPath.empty()) {
			...
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T("menu.issues.open_logs_tooltip", "Opens the PIXLRenderer.log file for manual review."));
			}
		}
		...
		if (IsVersionMismatchForCoreFeature(issue)) {
			...
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::TextWrapped("%s", T("menu.issues.core_feature_installed_tooltip",
											 "This feature is already included as part of the core PIXL Renderer installation. Uninstall this feature with your mod manager."));
			}
		} else if (issue.IsVersionMismatch()) {
			ImGui::SameLine();
		...
		if (canSafelyDelete) {
			...
			if (auto popup = Util::CenteredPopupModal(confirmPopupId.c_str())) {
				...
				if (issue.IsUnknown()) {
					...
					ImGui::BulletText("%s", T("menu.issues.reinstall_cs", "Consider reinstalling PIXL Renderer if issues persist"));
					ImGui::Spacing();
					ImGui::Separator();
					ImGui::Spacing();
				}
	...
	static bool IsVersionMismatchForCoreFeature(const FeatureIssueInfo& issue)
	{
		if (!issue.IsVersionMismatch())
			return false;
		RenderModule* f = s_featureLookupCache.FindFeature(issue.shortName);
		return f && f->IsCore();
	}

	bool IsReplacementFeatureInstalled(const std::string& featureName)
	{
		RenderModule* feature = s_featureLookupCache.FindFeature(featureName);
		return feature ? feature->loaded : false;
	}

	std::string PipelineHealth::GetModuleSupportLink(const std::string& featureName)
	{
		RenderModule* feature = s_featureLookupCache.FindFeature(featureName);
		if (feature && !feature->IsCore()) {
			return feature->GetModuleSupportLink();
		}
		return "";
	}
	...
	{
		std::filesystem::path featuresPath = Util::PathHelpers::GetModuleCatalogPath();
			...
			for (const auto& entry : std::filesystem::directory_iterator(featuresPath)) {
				if (entry.is_regular_file() && entry.path().extension() == ".ini") {
					...
					if (IsObsoleteFeature(featureName)) {
						...
						if (auto value = ini.GetValue("PIXL Module", "Version")) {
							version = value;
						}
						...
						AddFeatureIssue(featureName, "unknown",
							std::format("{} is not recognized by this PIXL build", featureName),
	...
	namespace Test
	{
		...
		{
			return Util::PathHelpers::GetModuleCatalogPath() / "PIXLWorkshopTestState.test";
		}
		...
		std::vector<TestIniInfo> CreateTestInis()
		{
			...
			const std::filesystem::path featuresPath = Util::PathHelpers::GetModuleCatalogPath();
			...
			const auto& loadedFeatures = RenderModule::GetModuleList();
			std::unordered_map<std::string, RenderModule*> loadedFeatureMap;
			...
			// We need to test all combinations: shader-breaking/non-shader-breaking Ã— with/without replacement
			...
			for (const auto& testCase : requiredTestCases) {
				...
				try {
					...
					std::string iniContent = std::format(
						...
						"\n"
						"[Settings]\n"
						"# Test INI created by PIXL Workshop for {}\n"
			...
			const std::string unknownFeature = "PIXLWorkshopTestUnknownFeature";
			...
			if (!std::filesystem::exists(unknownIniPath)) {
				try {
					std::string iniContent =
						...
						"\n"
						"[Settings]\n"
						"# Unknown test feature created by PIXL Workshop\n"
			...
			for (const auto& [featureName, feature] : loadedFeatureMap) {
				...
				std::string modLink = GetModuleSupportLink(featureName);
			...
			for (const auto& testFeatureName : testFeatures) {
				...
				try {
					if (candidate.hasExistingINI) {
						...
						ini.SetValue("PIXL Module", "Version", "0-0-1");
						...
						std::string iniContent =
							...
							"\n"
							"[Settings]\n"
							"# Test INI created by PIXL Workshop for version mismatch testing\n"
			...
			if (createdInis.empty()) {
				...
				logger::info("Created {} test INI files covering all major feature issue test cases. RenderModule issues will be detected immediately.", createdInis.size());
				...
				for (const auto& testInfo : createdInis) {
					...
					if (filename.find("ComplexParallax") != std::string::npos ||
						...
						filename.find("TerrainSeam") != std::string::npos) {
						obsoleteCount++;
					} else if (filename.find("PIXLWorkshopTestUnknown") != std::string::npos) {
						unknownCount++;
					} else if (!testInfo.isNewFile) {
		...
		bool RestoreOriginalState(const std::vector<TestIniInfo>& testInis)
		{
			...
			if (success) {
				logger::info("Successfully restored original state. RenderModule issues updated.");
			} else {
			...
			if (ImGui::CollapsingHeader(T("menu.issues.test.testing_header", "Testing"), ImGuiTreeNodeFlags_OpenOnArrow | ImGuiTreeNodeFlags_OpenOnDoubleClick)) {
				{
					auto sectionWrapper = Util::SectionWrapper(
						T("menu.issues.test.feature_issue_testing", "RenderModule Issue Testing"),
					...
					if (sectionWrapper) {
						...
						if (auto _tt = Util::HoverTooltipWrapper()) {
							ImGui::Text("%s", T("menu.issues.test.create_test_inis_tooltip",
												  ...
												  "- Obsolete features (ComplexParallaxMaterials, TerrainSeam, etc.)\n"
