...
enum class CachedShaderStage : std::uint8_t
{
	Vertex,
	Pixel,
	Compute,
...
struct RenderModule
{
	...
	// helpers and PIXL diagnostics without per-feature glue.
...
virtual const void* GetSettingsBlob() const { return nullptr; }
	...
	virtual std::string GetModuleSupportLink() { return ""; }
...
public:
	 ...
	 *
	 * The default deliberately follows the family-level HasShaderDefine contract. Modules
	 * with compile-time guards that restrict them to particular stages or descriptors may
	 * override this to avoid recompiling unrelated permutations in the same shader family.
	 */
	...
	{
		return ModuleVersions::FEATURE_CORE_NAMES.contains(const_cast<RenderModule*>(this)->GetShortName());
	}
	 ...
	 */
	virtual std::string_view GetCategory() const { return ModuleGroups::kOther; }

	/**
	 * Release maturity stage, declared via the feature .ini (Alpha/Beta flags) and
	 * baked into ModuleVersions.h at build time. Alpha takes precedence over Beta.
	 */
	...
	{
		const auto name = const_cast<RenderModule*>(this)->GetShortName();
		if (ModuleVersions::FEATURE_ALPHA_NAMES.contains(name))
			return ReleaseStage::Alpha;
		if (ModuleVersions::FEATURE_BETA_NAMES.contains(name))
	 ...
	 * feature .ini and baked into ModuleVersions.h at build time. Orthogonal to
	 * ReleaseStage: it gates UI visibility only, and carries no marker of its own.
	 */
	bool IsUnreleased() const { return ModuleVersions::FEATURE_UNRELEASED_NAMES.contains(const_cast<RenderModule*>(this)->GetShortName()); }
	 ...
	 */
	virtual std::pair<std::string, std::vector<std::string>> GetModuleSummary() { return {}; }
	 ...
	 *
	 * Validates the INI version against ModuleVersions, sets the loaded flag,
	 ...
	 * @return Whether the feature will be enabled at the next boot.
	 */
	 ...
	 */
	virtual std::vector<ModuleRules::Constraint> GetActiveConstraints() const { return {}; }
	...
	static const std::vector<RenderModule*>& GetModuleList();
	 ...
	 */
	static RenderModule* FindModuleById(const std::string& shortName);
	 ...
	 */
	static std::vector<std::string> GetLoadedModuleNames();

	// RenderModule utility functions
	/**
	 ...
	 */
	static std::string GetModuleRequiredVersion(const std::string& shortName);
	 ...
	 */
	static bool IsModuleKnown(const std::string& shortName, REL::Version* outVersion = nullptr);

	// Injected once from State after TracyD3D11Context is created so ForEachLoadedModule
	// can emit GPU timer zones without pulling in State headers here.
#ifdef TRACY_ENABLE
	inline static TracyD3D11Ctx s_tracyCtx = nullptr;

	/** @brief Sets the Tracy GPU context used by ForEachLoadedModule for GPU profiling zones. */
	 ...
	 * @param callback Callable receiving a RenderModule* for each loaded feature.
	 * @param emitGpuZone When true and Tracy is enabled, also emits a GPU timer zone.
	 */
	template <typename Func>
	static inline void ForEachLoadedModule(std::string_view methodName, Func&& callback, bool emitGpuZone = false)
	{
