	...
	static void thunk(RE::BSShader* shader, std::uintptr_t stream)
	{
		...
		if (shaderCache->IsDiskCache() || shaderCache->IsDump()) {
			if (shaderCache->IsDiskCache()) {
				RenderModule::ForEachLoadedModule("GenerateShaderPermutations", [shader](RenderModule* feature) {
...
namespace WeatherExtensions
{
	struct Sky_UpdateColors
	{
		...
		{
			func(sky, a_delta);
			globals::pipeline::skyContinuity.OnSkyUpdateColors(sky);
		}
		static inline REL::Relocation<decltype(thunk)> func;
	};
	...
	struct Sky_SetDirectionalAmbientColors
	{
		...
		{
			RE::NiColor values[3][2];
		};

		static void thunk(DirectionalAmbientColors& directionalAmbientColors, RE::NiColor* ambientSpecularTint, float ambientSpecularFresnel)
		{
...
struct IDXGISwapChain_Present
{
	...
	{
		globals::state->Reset();
		// The D3D12 compatibility swapchain paces itself immediately before its
		// own present. Native D3D11 uses the same PIXL limiter here.
		if (!globals::pipeline::imageReconstruction.d3d12SwapChainActive)
			globals::pipeline::imageReconstruction.FrameLimiter();

		HRESULT retval = globals::pipeline::cameraSuite.HandleSwapChainPresent(
			This,
			SyncInterval,
			Flags,
		...
		globals::pipeline::pixelCapture.ProcessCaptureRequest();
	...
	ID3D11DeviceContext** ppImmediateContext)
{
	...
	if (globals::pipeline::cameraSuite.loaded) {
		...
		CameraSuite::wasExclusiveFullscreen = !modifiedDesc.Windowed;
...
{
	// PIXL_DIRECTOR_EXCLUSIVE_INPUT_V3
	//
	// Director is a modal input context. It receives the full original event
	// stream through Menu::ProcessInputEvents, but Skyrim receives only the
	// small subset required to operate its native free camera.
	//
	// This prevents TAB / ESC / inventory / favorites / activate / combat /
	// controller face buttons from changing game state underneath Director.
	static bool IsDirectorFreeCameraEvent(RE::InputEvent* a_event)
	{
		if (!a_event)
			return false;

		// INPUT_DEVICE is an enum. Keyboard is value zero, so `!device`
		// incorrectly rejects every keyboard event before W/A/S/D are tested.
		const auto device =
			a_event->GetDevice();

		if (device ==
			RE::INPUT_DEVICES::INPUT_DEVICE::kKeyboard) {
			if (a_event->GetEventType() !=
				RE::INPUT_EVENT_TYPE::kButton)
				return false;

			auto* button =
				static_cast<RE::ButtonEvent*>(
					a_event);

			const std::uint32_t scanCode =
				button->GetIDCode();

			// Skyrim's keyboard ButtonEvent exposes DirectInput scan codes here.
			// Compare raw codes so native TFC movement reliably survives Director's
			// modal input lock.
			switch (scanCode) {
			case 0x11:  // W
			case 0x1E:  // A
			case 0x1F:  // S
			case 0x20:  // D
			case 0x10:  // Q (vertical on common TFC mappings)
			case 0x12:  // E (vertical on common TFC mappings)
		...
		if (device ==
			RE::INPUT_DEVICES::INPUT_DEVICE::kMouse) {
			// Mouse motion drives TFC look. Mouse buttons/wheel are ButtonEvent
			// input and stay private to Director so attacks/activate/menu actions
			// can never leak into the frozen game state.
			return
		...
		if (device ==
			RE::INPUT_DEVICES::INPUT_DEVICE::kGamepad) {
			// Sticks remain available to the native free camera. D-pad and all
			// face/shoulder buttons are consumed by Director or discarded.
			return
				a_event->GetEventType() ==
	...
	static void DispatchDirectorCameraEvents(
		RE::BSTEventSource<RE::InputEvent*>* a_dispatcher,
		RE::InputEvent* const* a_events)
	{
		if (!a_events ||
			!*a_events) {
			constexpr RE::InputEvent* const dummy[] = {
				nullptr
			};
			func(
				a_dispatcher,
		...
		// Build a temporary linked list from the original event nodes. Preserve
		// and restore every touched `next` pointer immediately after dispatch,
		// so chained input hooks see the original list unmodified.
		std::vector<RE::InputEvent*>
			allowed;
		std::vector<RE::InputEvent*>
			originalNext;
		...
		originalNext.reserve(
			allowed.size());

		for (std::size_t i = 0;
			 i < allowed.size();
			 ++i) {
			originalNext.push_back(
				allowed[i]->next);
...
namespace Hooks
{
	struct BSGraphics_Renderer_Init_InitD3D
	{
		static void thunk()
		{
			...
			CameraSuite::InstallSwapChainPresentHooks(globals::d3d::swapChain);
	...
	namespace CSShadersSupport
	{
		...
		struct Renderer_DispatchCSShader
		{
			static void thunk(RE::BSGraphics::Renderer* renderer, RE::BSGraphics::ComputeShader* shader, uint32_t threadGroupCountX, uint32_t threadGroupCountY, uint32_t threadGroupCountZ)
			{
				auto state = globals::state;
				auto shaderCache = globals::shaderCache;
				auto& vl = globals::pipeline::lightVolumes;
	...
	struct BSLightingShader_SetupGeometry_GeometrySetupConstantPointLights
	{
		static void thunk(RE::BSGraphics::PixelShader* PixelShader, RE::BSRenderPass* Pass, DirectX::XMMATRIX& Transform, uint32_t LightCount, uint32_t ShadowLightCount, float WorldScale, uint32_t)
		{
			if (globals::pipeline::radiantGrid.loaded)
				globals::pipeline::radiantGrid.BSLightingShader_SetupGeometry_GeometrySetupConstantPointLights(Pass);
			else
