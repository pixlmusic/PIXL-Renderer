...
namespace FrameAnnotations
{
	...
	struct BSShader_SetupGeometry
	{
		static void thunk(RE::BSShader* shader, RE::BSRenderPass* pass, uint32_t renderFlags)
		{
			if (globals::state->frameAnnotations) {
				...
				const uint32_t shard = (descriptor ^ (descriptor >> 8) ^ (descriptor >> 16) ^ (descriptor >> 24)) & 0xffu;
				std::string diskPath = std::format("Data/PIXL/PipelineLibrary/Pixel/{:02X}/{}/{:08X}{}.pixlbin", shard, shader->fxpFilename, descriptor, definesSuffix);
	...
	struct Main_RenderWaterEffects
	{
		static void thunk()
		{
			globals::state->BeginPerfEvent("Water Optics");
	...
	struct VolumetricLightingDescriptor_Render
	{
		...
		{
			globals::state->BeginPerfEvent("Light Volumes");
