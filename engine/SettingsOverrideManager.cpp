...
size_t SettingsOverrideManager::DiscoverOverrides()
{
	...
	try {
		for (const auto& entry : std::filesystem::directory_iterator(overridesDir)) {
			...
			if (ec) {
				logger::warn("Could not get size of override file, skipping: {}", entry.path().string());
				continue;
			}
			...
			try {
				...
				if (overrideInfo) {
					...
					if (!override.isGlobal) {
						...
						if (!override.featureName.empty() && override.featureName.length() <= MAX_STRING_LENGTH) {
							...
							logger::warn("Override has invalid feature name, skipping feature mapping: {}", override.modName);
						}
					}
				...
				logger::warn("Error loading override file {}: {}", entry.path().string(), e.what());

				// Report to RenderModule Issues
				auto [modName, featureName] = ParseOverrideFilename(entry.path().filename().string());
				if (!modName.empty()) {
		...
		logger::warn("Unexpected error during override discovery: {}", e.what());
	}
		...
		for (size_t index : it->second) {
			...
			if (override.enabled) {
				try {
					...
					// Report application failure to RenderModule Issues
...
json SettingsOverrideManager::LoadAppliedOverridesTracking() const
{
	...
	try {
		...
		if (ec) {
			logger::warn("Could not get size of applied overrides tracking file: {}", trackingPath.string());
			return appliedOverrides;
		}
		...
		if (file.is_open()) {
			try {
				...
				if (!appliedOverrides.is_object()) {
					logger::warn("Applied overrides tracking file contains invalid data structure, resetting");
					...
					while (it != appliedOverrides.end()) {
						...
						// Invalid entry
						logger::warn("Invalid tracking entry for '{}', removing", key);
						it = appliedOverrides.erase(it);
					}
				}
			} catch (const json::parse_error& e) {
				logger::warn("Parse error in applied overrides tracking file: {} at byte {}", e.what(), e.byte);
				appliedOverrides = json::object();
			} catch (const json::exception& e) {
				logger::warn("JSON error reading applied overrides tracking file: {}", e.what());
				appliedOverrides = json::object();
			}
		}
	} catch (const std::filesystem::filesystem_error& e) {
		logger::warn("Filesystem error loading applied overrides tracking: {}", e.what());
	} catch (const std::exception& e) {
		logger::warn("Error loading applied overrides tracking: {}", e.what());
	}
	return appliedOverrides;
}
...
void SettingsOverrideManager::SaveAppliedOverridesTracking(const json& appliedOverrides) const
{
	try {
		...
		if (std::filesystem::exists(trackingPath, ec) && !ec) {
			...
			if (ec) {
				logger::warn("Could not create backup of tracking file: {}", ec.message());
			}
		}
		...
		if (file.is_open()) {
			try {
				...
				if (file.fail()) {
					logger::warn("Failed to write applied overrides tracking file completely");
				}
			} catch (const json::exception& e) {
				logger::warn("JSON error writing applied overrides tracking file: {}", e.what());
			}
		} else {
			logger::warn("Could not open applied overrides tracking file for writing: {}", trackingPath.string());
		}
	} catch (const std::filesystem::filesystem_error& e) {
...
std::unique_ptr<SettingsOverrideManager::OverrideInfo> SettingsOverrideManager::LoadOverrideFile(const std::filesystem::path& filePath)
{
	try {
		...
		if (ec) {
			logger::warn("Could not get file size for override file: {}", filePath.string());
			return nullptr;
		}
		...
		try {
			...
			logger::warn("JSON parse error in override file {}: {} at byte {}",
				filePath.string(), e.what(), e.byte);
			return nullptr;
		} catch (const json::exception& e) {
			logger::warn("JSON error in override file {}: {}", filePath.string(), e.what());
			return nullptr;
		}
		...
		if (!ValidateJsonDataTypes(overrideJson, "", filePath.string())) {
			logger::warn("Invalid data types in override file: {}", filePath.string());
			return nullptr;
		}
		...
		if (modName.empty() || modName.length() > MAX_STRING_LENGTH) {
			logger::warn("Invalid mod name in override file: {}", filePath.string());
			return nullptr;
		}

		if (!featureName.empty() && featureName.length() > MAX_STRING_LENGTH) {
			logger::warn("Invalid feature name in override file: {}", filePath.string());
			return nullptr;
		}
		...
		if (overrideJson.contains("_metadata") && overrideJson["_metadata"].is_object()) {
			...
			try {
				...
				logger::warn("Error parsing metadata in override file {}: {}", filePath.string(), e.what());
		...
		logger::warn("Filesystem error loading override file {}: {}", filePath.string(), e.what());
		return nullptr;
	} catch (const std::exception& e) {
		logger::warn("Unexpected error loading override file {}: {}", filePath.string(), e.what());
		return nullptr;
	}
}
...
bool SettingsOverrideManager::ValidateOverrideFormat(const json& overrideJson, const std::string& filePath)
{
	try {
		...
		if (!filePath.empty()) {
			logger::warn("JSON error during validation of override file {}: {}", filePath, e.what());
		}
		return false;
	} catch (const std::exception& e) {
		if (!filePath.empty()) {
			logger::warn("Error during validation of override file {}: {}", filePath, e.what());
		}
		return false;
	}
}
...
bool SettingsOverrideManager::ValidateJsonDataTypes(const json& jsonData, const std::string& path, const std::string& filePath)
{
	try {
		...
		if (!filePath.empty()) {
			logger::warn("JSON error during data type validation at '{}' in override file {}: {}", path, filePath, e.what());
		}
		return false;
	} catch (const std::exception& e) {
		if (!filePath.empty()) {
			logger::warn("Error during data type validation at '{}' in override file {}: {}", path, filePath, e.what());
		}
		return false;
	}
}
...
json SettingsOverrideManager::SanitizeJsonData(const json& jsonData)
{
	try {
		...
		logger::warn("Error during JSON sanitization: {}", e.what());
	...
	try {
		...
		for (const auto& [key, value] : override.items()) {
			...
			try {
				if (target.contains(key) && target[key].is_object() && value.is_object()) {
					...
					if (value.is_string()) {
						...
						if (std::isnan(numValue) || std::isinf(numValue)) {
							logger::warn("Skipping merge of key '{}' - invalid numeric value", key);
							continue;
						}
				...
				logger::warn("JSON error merging key '{}': {}", key, e.what());
				continue;
			} catch (const std::exception& e) {
				logger::warn("Error merging key '{}': {}", key, e.what());
				continue;
			}
		}
	} catch (const json::exception& e) {
		logger::warn("JSON error during merge operation: {}", e.what());
	} catch (const std::exception& e) {
		logger::warn("Error during merge operation: {}", e.what());
	}
}
...
void SettingsOverrideManager::ReportOverrideFailure(const std::string& modName, const std::string& featureName, const std::string& errorMessage)
{
	...
	PipelineHealth::FeatureFileInfo fileInfo;
	...
	// Add to RenderModule Issues as an override failure
	PipelineHealth::AddFeatureIssue(
		modName + (featureName.empty() ? "_Global" : "_" + featureName),  // Use combined name as shortName
		"unknown",                                                        // version
		fullErrorMessage,
		PipelineHealth::FeatureIssueInfo::IssueType::OVERRIDE_FAILED,
		fileInfo);
}
...
bool SettingsOverrideManager::LoadUserOverride(const std::string& featureName, json& featureJson)
{
	...
	try {
		...
		if (ec || fileSize == 0 || fileSize > 1024 * 1024) {
			logger::warn("User override file invalid size: {}", userFilePath.string());
			return false;
		}
		...
		logger::warn("Error loading user override for {}: {}", featureName, e.what());
		return false;
	}
}
...
bool SettingsOverrideManager::SaveUserOverride(const std::string& featureName, const json& currentSettings, const json& overrideSettings)
{
	...
	try {
		...
		if (!file.is_open()) {
			logger::warn("Could not create user override file: {}", userFilePath.string());
			return false;
		}
		...
		if (file.fail()) {
			logger::warn("Failed to write user override file: {}", userFilePath.string());
			file.close();
			return false;
		}
...
bool SettingsOverrideManager::DeleteUserOverride(const std::string& featureName)
{
	...
	try {
		...
		if (ec) {
			logger::warn("Failed to delete user override file: {}", ec.message());
			return false;
		}
