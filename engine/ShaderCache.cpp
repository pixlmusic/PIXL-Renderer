...
namespace SIE
{
	...
	namespace SShaderCache
	{
		...
		*/
		...
		static void GetLightingShaderDefines(uint32_t descriptor, std::span<D3D_SHADER_MACRO> defines)
		{
			...
			if ((descriptor & static_cast<uint32_t>(ShaderCache::LightingShaderFlags::MaterialForge)) != 0) {
				defines[lastIndex++] = { "MATERIAL_FORGE", nullptr };
		...
		std::wstring GetDiskPath(const std::string_view& name, uint32_t descriptor, ShaderClass shaderClass)
		{
			...
			// PIXL stores permutations by pipeline stage and a descriptor shard. This
			// avoids thousands of files in inherited flat shader-name directories and
			// makes cache inspection follow the same stage-first model as compilation.
			const auto shard = (descriptor ^ (descriptor >> 8) ^ (descriptor >> 16) ^ (descriptor >> 24)) & 0xffu;
			switch (shaderClass) {
			case ShaderClass::Pixel:
				return std::format(L"Data/PIXL/PipelineLibrary/Pixel/{:02X}/{}/{:08X}{}.pixlbin", shard, wname, descriptor, suffix);
			case ShaderClass::Vertex:
				return std::format(L"Data/PIXL/PipelineLibrary/Vertex/{:02X}/{}/{:08X}{}.pixlbin", shard, wname, descriptor, suffix);
			case ShaderClass::Compute:
				return std::format(L"Data/PIXL/PipelineLibrary/Compute/{:02X}/{}/{:08X}{}.pixlbin", shard, wname, descriptor, suffix);
			}
			return {};
		}
		...
		static ID3DBlob* CompileShader(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor, bool useDiskCache, ShaderFileDependencyTracker* dependencyTracker)
		{
			...
			// compile shaders â€” match Utils/D3D.cpp CompileShader flag policy (strictness, optional toggles, validation).
	...
	void ShaderCache::DeleteDiskCache(std::string_view a_reason)
	{
		...
		try {
			...
			logger::info("Deleted PIXL pipeline library (reason: {})", a_reason);
		} catch (std::filesystem::filesystem_error const& ex) {
			logger::error("Failed to delete disk cache (reason: {}): {}", a_reason, ex.what());
		}
	}

	void ShaderCache::InvalidateDiskCache(RE::BSShader::Type a_type, std::string_view a_reason)
	{
		std::vector<std::string> families;
		if (a_type == RE::BSShader::Type::Grass) {
			families.emplace_back("RunGrass");
		} else if (a_type == RE::BSShader::Type::ImageSpace) {
			families.emplace_back("IS*");
			families.emplace_back("ReflectionsRayTracing");
		} else {
			auto family = magic_enum::enum_name(a_type);
			if (!family.empty())
		...
		if (families.empty()) {
			logger::warn(
				"Cannot map shader type {} to a persistent pipeline family (reason: {}); disk cache preserved",
		...
		const std::filesystem::path cacheRoot{ L"Data/PIXL/PipelineLibrary" };
		std::scoped_lock lock{ compilationSet.compilationMutex };
		try {
			for (const auto* stageName : { L"Vertex", L"Pixel", L"Compute" }) {
				const auto stageRoot = cacheRoot / stageName;
				if (!std::filesystem::exists(stageRoot))
					continue;

				for (const auto& shard : std::filesystem::directory_iterator(stageRoot)) {
					if (!shard.is_directory())
						continue;

					for (const auto& family : families) {
						if (family == "IS*") {
							for (const auto& candidate : std::filesystem::directory_iterator(shard.path())) {
								if (!candidate.is_directory() || !candidate.path().filename().string().starts_with("IS"))
									continue;
								for (const auto& file : std::filesystem::recursive_directory_iterator(candidate.path())) {
									if (file.is_regular_file()) {
										++removedFiles;
										removedBytes += file.file_size();
									}
								}
						...
						const auto target = shard.path() / family;
			...
			logger::error(
				"Selective pipeline invalidation failed for {} (reason: {}): {}",
		...
		logger::info(
			"Selectively invalidated {} {} stages ({} bytes; reason: {})",
			removedFiles,
			magic_enum::enum_name(a_type),
			removedBytes,
			a_reason);
	...
	void ShaderCache::ValidateDiskCache()
	{
		...
		if (!invalidModules.empty()) {
			...
			std::vector<InvalidationRule> rules;
			bool requiresFullInvalidation = false;

			const auto addRule = [&](RenderModule* module, RE::BSShader::Type shaderType, std::string_view family, bool prefixMatch = false) {
				rules.push_back({ module, shaderType, std::string(family), prefixMatch });
			};

			for (auto* module : invalidModules) {
				bool moduleAffectsPipeline = false;
				for (int shaderTypeValue = static_cast<int>(RE::BSShader::Type::Grass);
					 shaderTypeValue < static_cast<int>(RE::BSShader::Type::Total);
					 ++shaderTypeValue) {
					const auto shaderType = static_cast<RE::BSShader::Type>(shaderTypeValue);
					if (!module->HasShaderDefine(shaderType))
						continue;

					moduleAffectsPipeline = true;
					switch (shaderType) {
					case RE::BSShader::Type::Grass:
						addRule(module, shaderType, "RunGrass");
						break;
					case RE::BSShader::Type::ImageSpace:
						// Image-space descriptors use many IS* filenames rather than one
						// directory. Mark the prefix and the standalone reflection family.
						addRule(module, shaderType, "IS", true);
						addRule(module, shaderType, "ReflectionsRayTracing");
						break;
					default: {
						auto family = magic_enum::enum_name(shaderType);
						if (family.empty())
							requiresFullInvalidation = true;
						else
							addRule(module, shaderType, family);
						break;
					}
					}
				}

				// An unloaded module can make HasShaderDefine state-dependent, so a
				// cached-enabled -> unloaded transition without a discoverable family
			...
			if (requiresFullInvalidation) {
				logger::info("Disk cache module change could not be narrowed safely; performing full invalidation");
				DeleteDiskCache("module change could not be mapped to shader families");
				return;
			}

			std::uintmax_t removedFiles = 0;
			std::uintmax_t removedBytes = 0;
			std::uintmax_t preservedFiles = 0;
			std::uintmax_t unparsableFiles = 0;
			try {
				for (const auto* stageName : { L"Vertex", L"Pixel", L"Compute" }) {
					const auto stageRoot = cacheRoot / stageName;
					if (!std::filesystem::exists(stageRoot))
						continue;
					const auto stage =
						std::wstring_view(stageName) == L"Vertex" ? CachedShaderStage::Vertex :
						std::wstring_view(stageName) == L"Pixel" ? CachedShaderStage::Pixel :
						CachedShaderStage::Compute;

					for (const auto& shard : std::filesystem::directory_iterator(stageRoot)) {
						if (!shard.is_directory())
							continue;

						for (const auto& familyDirectory : std::filesystem::directory_iterator(shard.path())) {
							if (!familyDirectory.is_directory())
								continue;

							const auto familyName = familyDirectory.path().filename().string();
							const auto familyMatches = [&](const InvalidationRule& rule) {
								return rule.prefixMatch ? familyName.starts_with(rule.family) : familyName == rule.family;
							};
							if (std::ranges::none_of(rules, familyMatches))
								continue;

							for (const auto& file : std::filesystem::recursive_directory_iterator(familyDirectory.path())) {
								if (!file.is_regular_file())
									continue;
								...
								if (filename.size() >= 8) {
									const auto* begin = filename.data();
									const auto [end, error] = std::from_chars(begin, begin + 8, descriptor, 16);
									parsedDescriptor = error == std::errc{} && end == begin + 8;
								}

								bool removeEntry = !parsedDescriptor;
								if (!parsedDescriptor)
									++unparsableFiles;
								for (const auto& rule : rules) {
									if (familyMatches(rule) &&
										rule.module->AffectsCachedShader(rule.shaderType, descriptor, stage)) {
										removeEntry = true;
										break;
									}
								}
								...
								if (removeEntry) {
									...
									++preservedFiles;
									// A changed module may share a top-level .hlsl file with unrelated
									// permutations. Re-stamp only entries which every changed module
									// explicitly declared unaffected, preventing the coarse source-mtime
									// fallback from recompiling the whole family immediately afterwards.
									if (isSkipUnchangedShaders)
										std::filesystem::last_write_time(file.path(), std::filesystem::file_time_type::clock::now());
								}
							}
						}
					}
				...
				logger::error("Selective pipeline invalidation failed: {}", ex.what());
				DeleteDiskCache("selective startup invalidation raised a filesystem error");
				return;
			}

			logger::info(
				"Selectively invalidated {} cached shader stages ({} bytes) for {} changed module(s); revalidated {} unaffected stages in shared families",
				removedFiles,
				removedBytes,
				invalidModules.size(),
				preservedFiles);
			if (unparsableFiles > 0) {
				logger::warn(
					"Removed {} cache entries with names that did not match the PIXL descriptor format",
					unparsableFiles);
			}
		} else {
	...
	void ShaderCache::ProcessCompilationSet(std::stop_token stoken, SIE::ShaderCompilationTask task)
	{
		...
		{
			// GetString() format: "fxpFilename:ShaderClass:defines" â€” filename is before the first colon.
	...
	int ShaderCompilationTask::ComputePriority(ShaderClass shaderClass, const RE::BSShader& shader, uint32_t descriptor)
	{
		...
		// Base priority by shader type â€” Lighting is consistently the slowest
		...
		// More active descriptor bits â†’ more #defines â†’ more code paths for the compiler
		...
		if (type == RE::BSShader::Type::Lighting) {
			...
			// due to multi-texture blending codegen â€” regularly 60-130s compile times
			...
			// MATERIAL_FORGE and ANISO_LIGHTING are the dominant cost drivers,
			// especially in combination with LANDSCAPE (115-130s observed)
			if (descriptor & static_cast<uint32_t>(ShaderCache::LightingShaderFlags::MaterialForge))
				priority += 500;
			...
			// LANDSCAPE + MATERIAL_FORGE combination triggers extreme register pressure
			// (6x unrolled texture layers * PBR params = 30+ textures, 180s+ compile)
			if ((technique == ShaderCache::LightingShaderTechniques::MTLand ||
					technique == ShaderCache::LightingShaderTechniques::MTLandLODBlend) &&
				(descriptor & static_cast<uint32_t>(ShaderCache::LightingShaderFlags::MaterialForge)))
				priority += 500;
		}
	...
	std::optional<ShaderCompilationTask> CompilationSet::WaitTake(std::stop_token stoken)
	{
		...
		// PIXL compile waves: establish a playable core image first, then material
		// coverage, then atmospheric/effect permutations. Within each wave we still
		// use longest-processing-time ordering to control the tail.
		auto bestIt = availableTasks.end();
		for (uint32_t search = 0; search < 3 && bestIt == availableTasks.end(); ++search) {
			const uint32_t wave = (pixlCompileWave + search) % 3u;
			for (auto it = availableTasks.rbegin(); it != availableTasks.rend(); ++it) {
				const auto type = it->GetShaderType();
				const uint32_t taskWave =
					(type == RE::BSShader::Type::ImageSpace || type == RE::BSShader::Type::Lighting) ? 0u :
					(type == RE::BSShader::Type::Grass || type == RE::BSShader::Type::DistantTree || type == RE::BSShader::Type::Water) ? 1u : 2u;
				if (taskWave == wave) {
					bestIt = std::prev(it.base());
					pixlCompileWave = (wave + 1u) % 3u;
					break;
				}
			}
		}
	...
	void UpdateListener::UpdateCache(const std::filesystem::path& filePath, SIE::ShaderCache* cache, bool& clearCache, bool& fileDone)
	{
		...
		if (!shaderType.has_value() && _stricmp(shaderTypeString.c_str(), "RunGrass") == 0)
			shaderType = RE::BSShader::Type::Grass;

		auto findOwningModule = [&]() -> RenderModule* {
			std::error_code ec;
			auto shaderRoot = std::filesystem::weakly_canonical("Data\\Shaders", ec);
			if (ec)
				return nullptr;
			auto absolutePath = std::filesystem::weakly_canonical(filePath, ec);
			if (ec)
				return nullptr;
			auto relativePath = std::filesystem::relative(absolutePath, shaderRoot, ec);
			if (ec || relativePath.empty())
				return nullptr;
			auto component = relativePath.begin();
			if (component == relativePath.end())
				return nullptr;
			return RenderModule::FindModuleById(component->string());
		};

		auto invalidateModuleFamilies = [&](RenderModule* module, std::string_view reason) {
			if (!module)
				return false;
			bool affectedPipeline = false;
			for (int shaderTypeValue = static_cast<int>(RE::BSShader::Type::Grass);
				 shaderTypeValue < static_cast<int>(RE::BSShader::Type::Total);
				 ++shaderTypeValue) {
				const auto type = static_cast<RE::BSShader::Type>(shaderTypeValue);
				if (!module->HasShaderDefine(type))
					continue;
				affectedPipeline = true;
				cache->InvalidateDiskCache(type, reason);
				cache->Clear(type);
			}
			module->ClearShaderCache();
			return affectedPipeline;
		};
		...
		if (!std::filesystem::is_directory(filePath) && lowerExtension == ".hlsl") {
			...
			if (!foundPath) {
				...
				if (parentDirName == "shaders" && shaderType.has_value()) {
					const std::string reason = std::format(
						"file watcher changed top-level shader {}", filePath.string());
					cache->InvalidateDiskCache(shaderType.value(), reason);
					cache->Clear(shaderType.value());
					logger::info(
						"Shader hot reload narrowed {} to {} only",
						filePath.string(),
						magic_enum::enum_name(shaderType.value()));
				} else {
					// Nested .hlsl files are directly compiled feature kernels, not game
					// pipeline families. Releasing the owning module's kernels is enough;
					// deleting thousands of unrelated Lighting/Grass stages was the source
					// of the surprise full rebuild after roof-runoff iteration.
					auto* owner = findOwningModule();
					if (owner) {
						owner->ClearShaderCache();
						logger::info(
							"Shader hot reload released direct kernels for {} after {} changed; persistent pipeline preserved",
							owner->GetShortName(),
							filePath.string());
					} else {
						logger::warn(
							"Shader hot reload could not identify an owning module for {}; persistent pipeline preserved (restart may be required)",
							filePath.string());
					}
				}
			}
		...
		// Handle include file changes (.hlsli) by invalidating dependents
			...
			if (owner) {
				const std::string reason = std::format(
					"file watcher changed {} include {}", owner->GetShortName(), filePath.string());
				invalidateModuleFamilies(owner, reason);
				logger::info(
					"Shader hot reload invalidated only {} pipeline families after include change {}",
					owner->GetShortName(),
					filePath.string());
			} else if (dependents.empty()) {
				// A changed unowned shared include with no dependency graph really can
				// affect every family. Keep the conservative fallback, but make the
				// triggering path explicit in the log.
				logger::warn(
					"Shared shader include {} has no tracked dependents or owning module; full pipeline invalidation required",
					filePath.string());
				clearCache = true;
			}
		}
	...
	void UpdateListener::processQueue()
	{
		...
		while (cache->UseFileWatcher()) {
			...
			if (!queue.empty()) {
				...
				if (clearCache) {
					cache->DeleteDiskCache("file watcher changed an unowned shared include with no dependency graph");
					cache->Clear();
				}
				queue.clear();
			}
	...
	void UpdateListener::handleFileAction(efsw::WatchID watchid, const std::string& dir, const std::string& filename, efsw::Action action, std::string oldFilename)
	{
		...
		if (queue.empty() ||
			queue.back().action != action ||
			queue.back().filename != filename ||
			queue.back().dir != dir) {
