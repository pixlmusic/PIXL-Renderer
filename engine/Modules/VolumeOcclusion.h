...
struct VolumeOcclusion : RenderModule
{
public:
	virtual inline std::string GetName() override { return "Volume Occlusion"; }
	virtual std::string GetDisplayName() override { return T("feature.volume_occlusion.name", "Volume Occlusion"); }
	/** @brief Returns the short identifier used for file paths and logging. */
	virtual inline std::string GetShortName() override { return "VolumeOcclusion"; }
	virtual inline std::string_view GetShaderDefineName() override { return "VOLUME_OCCLUSION"; }
	...
	{
		return { T("feature.volume_occlusion.description", "Volume Occlusion provides downsampled VSM shadow maps for use by effects like particles and decals.\nThis improves shadow quality on transparent objects with minimal performance impact."),
			{ T("feature.volume_occlusion.key_feature_1", "Downsampled VSM shadows"),
				T("feature.volume_occlusion.key_feature_2", "Gaussian blur filtering"),
				T("feature.volume_occlusion.key_feature_3", "Multi-cascade support"),
				T("feature.volume_occlusion.key_feature_4", "Optimized for effects rendering") } };
	};
