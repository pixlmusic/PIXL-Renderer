/**
 * @file PulseProfiler.cpp
 * @brief Real-time performance monitoring system for Skyrim PIXL Renderer
 *
...
#include "PulseProfiler.h"
#include "RenderModule.h"
#include "Modules/PulseProfiler/ABTesting/ABTestAggregator.h"
...
#define I18N_KEY_PREFIX "feature.pulse_profiler."
...
// ============================================================================
// VIRTUAL OVERRIDES (RenderModule.h interface)
// ============================================================================

void PulseProfiler::DrawSettings()
{
	...
	PIXLUI::SectionBanner("RESOURCE MONITOR");
	ImGui::TextColored(
		PIXLUI::ToVec4(PIXLUI::Colors::TextDim),
		"Live frame pacing, renderer workload and video-memory telemetry.");
	ImGui::Dummy(ImVec2(0.0f, PIXLUI::Ref(4.0f)));

	if (PIXLUI::LabeledToggle("Show resource monitor", &this->settings.ShowInOverlay)) {
		// UI-only setting; existing persistence path remains unchanged.
	}
	if (ImGui::IsItemHovered()) {
		if (auto _tt = Util::HoverTooltipWrapper()) {
			ImGui::Text("Toggle with %s", Util::Input::KeyIdToString(menuSettings.OverlayToggleKey).c_str());
		}
	}
	...
	PIXLUI::SectionBanner("SIGNALS");
	PIXLUI::LabeledToggle("FPS + frame pacing", &this->settings.ShowFPS);
	PIXLUI::LabeledToggle("Engine workloads", &this->settings.ShowDrawCalls);
	PIXLUI::LabeledToggle("PIXL GPU passes", &this->settings.ShowCSPasses);
	PIXLUI::LabeledToggle("Video memory", &this->settings.ShowVRAM);

	const bool frameGenerationActive =
		globals::pipeline::imageReconstruction.IsFrameGenerationActive();
	if (this->settings.ShowFPS) {
		PIXLUI::LabeledToggle(
			frameGenerationActive ? "Render-cadence graph" : "Frame-time graph",
			&this->settings.ShowPreFGFrameTimeGraph);
		if (frameGenerationActive) {
			PIXLUI::LabeledToggle(
				"Presented-frame graph",
				&this->settings.ShowPostFGFrameTimeGraph);
		}
	}

	PIXLUI::SectionBanner("PRESENTATION");
	...
	PIXLUI::SliderFloatField(
		...
		1.0f,
		"%.2f");
	PIXLUI::LabeledToggle("Frame border", &this->settings.ShowBorder);
	PIXLUI::SliderFloatField(
		"Telemetry interval",
		&this->settings.UpdateInterval,
		0.05f,
		PulseProfiler::Settings::kMaxUpdateInterval,
		"%.2f s");
	PIXLUI::SliderIntField(
		"History samples",
		&this->settings.FrameHistorySize,
		this->settings.kMinFrameHistorySize,
		this->settings.kMaxFrameHistorySize);

	ImGui::Dummy(ImVec2(0.0f, PIXLUI::Ref(5.0f)));
	if (PIXLUI::ActionButton("RESET POSITION", ImVec2(PIXLUI::Ref(146.0f), PIXLUI::Ref(30.0f)))) {
		this->settings.PositionSet = false;
	}
	ImGui::SameLine();
	if (PIXLUI::ActionButton("RESTORE DEFAULTS", ImVec2(PIXLUI::Ref(162.0f), PIXLUI::Ref(30.0f)))) {
...
void PulseProfiler::LoadSettings(json& j)
{
	try {
		// Load all settings from JSON (missing fields use defaults)
		this->settings = j.get<PulseProfiler::Settings>();
	} catch (...) {
		// Fallback to defaults if JSON is invalid
		this->settings = PulseProfiler::Settings{};
	}
...
void PulseProfiler::RestoreDefaultSettings()
{
	this->settings = PulseProfiler::Settings{};
...
void PulseProfiler::DrawOverlay()
{
	auto* menu = Menu::GetSingleton();
	if (!globals::state || !menu || !menu->overlayVisible || !ImGui::GetCurrentContext() || !this->settings.ShowInOverlay)
		return;

	auto [mainRows, summaryRows] = this->BuildDrawCallRows();
	std::erase_if(mainRows, [](const DrawCallRow& row) { return !row.enabled || row.frameTime <= 0.0f; });
	std::ranges::sort(mainRows, std::greater{}, &DrawCallRow::frameTime);

	struct PipelinePassRow
	{
		std::string label;
	...
	std::vector<PipelinePassRow> pipelinePasses;
	pipelinePasses.reserve(globals::profiler ? globals::profiler->GetResults().size() : 0);
	float pixlTime = 0.0f;
	float pixlAverage = 0.0f;
	if (globals::profiler) {
		for (const auto& result : globals::profiler->GetResults()) {
			if (!result.valid || result.avgMs <= 0.0f)
				continue;

			std::string label = result.name;
			const auto separator = result.name.find("::");
			if (separator != std::string::npos) {
				const auto module = std::string_view(result.name).substr(0, separator);
				const auto pass = std::string_view(result.name).substr(separator + 2);
				label = std::format("{} / {}", PIXLRendererPage::GetPublicName(module, module), pass);
			}

			pipelinePasses.push_back({ std::move(label), result.gpuTimeMs, result.avgMs, result.p95Ms });
			pixlTime += std::max(result.gpuTimeMs, 0.0f);
			pixlAverage += std::max(result.avgMs, 0.0f);
		}
	}
	std::ranges::sort(pipelinePasses, std::greater{}, &PipelinePassRow::averageMs);

	const float scale = Util::GetUIScale();
	const float minWidth = 500.0f * scale;
	const float minHeight = 540.0f * scale;
	if (!this->settings.PositionSet) {
		const float pad = Settings::kDefaultWindowPadding * scale;
		this->settings.Position = { pad, pad };
		this->settings.PositionSet = true;
	}

	ImGui::SetNextWindowPos(this->settings.Position, ImGuiCond_FirstUseEver);
	ImGui::SetNextWindowSize({ 520.0f * scale, 590.0f * scale }, ImGuiCond_FirstUseEver);
	...
	ImGui::PushStyleColor(
		ImGuiCol_WindowBg,
		ImVec4(0.027f, 0.038f, 0.044f, std::clamp(this->settings.BackgroundOpacity, 0.35f, 1.0f)));
	ImGui::PushStyleColor(ImGuiCol_Border, PIXLUI::ToVec4(PIXLUI::Colors::BorderBright));
	...
	if (ImGui::Begin("##PIXLResourceMonitor", nullptr, windowFlags)) {
		this->settings.Position = ImGui::GetWindowPos();
		...
		draw->AddLine(
			ImVec2(windowPos.x + 2.0f * scale, windowPos.y + 6.0f * scale),
			ImVec2(windowPos.x + 2.0f * scale, windowPos.y + 49.0f * scale),
			PIXLUI::Colors::Cyan,
			2.0f * scale);
		draw->AddLine(
			ImVec2(windowPos.x + 2.0f * scale, windowPos.y + 6.0f * scale),
			ImVec2(windowPos.x + 54.0f * scale, windowPos.y + 6.0f * scale),
			PIXLUI::Colors::CyanSoft,
			1.0f * scale);

		ImGui::TextColored(PIXLUI::ToVec4(PIXLUI::Colors::Text), "PIXL / RESOURCE MONITOR");
		const char* hotkey = "F10";
		...
		if (this->settings.ShowFPS) {
			const float frameMs = this->state.smoothFrameTimeMs;
			const ImU32 paceU32 = frameMs <= 8.34f ? PIXLUI::Colors::Success :
				frameMs <= 16.7f ? PIXLUI::Colors::Warning : PIXLUI::Colors::Danger;
			ImGui::TextColored(PIXLUI::ToVec4(paceU32), "%.1f FPS", this->state.smoothFps);
			ImGui::SameLine(118.0f * scale);
			ImGui::Text("%.2f ms", frameMs);
			ImGui::SameLine(205.0f * scale);
			ImGui::TextDisabled("PIXL %.2f ms  /  AVG %.2f ms", pixlTime, pixlAverage);
			if (this->state.isFrameGenerationActive)
				ImGui::TextDisabled("PRESENTED %.1f FPS   /   RENDER CADENCE %.1f FPS", this->state.postFGSmoothFps, this->state.smoothFps);
		}
		...
		auto drawSignalRow = [scale](const char* label, float value, float reference, const char* suffix) {
			const ImVec2 start = ImGui::GetCursorScreenPos();
			const float width = ImGui::GetContentRegionAvail().x;
			const float height = 25.0f * scale;
			const float metricWidth = 116.0f * scale;
			const float labelWidth = std::clamp(width * 0.52f, 180.0f * scale, width - metricWidth - 50.0f * scale);
			const float barX = start.x + labelWidth;
			const float barWidth = std::max(64.0f * scale, width - labelWidth - metricWidth);
			const float barY = start.y + height - 5.0f * scale;
			ImDrawList* dl = ImGui::GetWindowDrawList();
			dl->AddText(start, PIXLUI::Colors::TextMuted, label);
			dl->AddLine(ImVec2(barX, barY), ImVec2(barX + barWidth, barY), PIXLUI::Colors::BorderSoft, 3.0f * scale);
			const float fill = reference > 0.0001f ? std::clamp(value / reference, 0.0f, 1.0f) : 0.0f;
			dl->AddLine(ImVec2(barX, barY), ImVec2(barX + barWidth * fill, barY), PIXLUI::Colors::CyanSoft, 3.0f * scale);
			char metric[96]{};
			std::snprintf(metric, sizeof(metric), "%.2f %s", value, suffix);
			const ImVec2 metricSize = ImGui::CalcTextSize(metric);
			dl->AddText(ImVec2(start.x + width - metricSize.x, start.y), PIXLUI::Colors::Text, metric);
		...
		if (this->settings.ShowCSPasses) {
			...
			ImGui::TextColored(
				PIXLUI::ToVec4(
					PIXLUI::Colors::TextDim),
				"Relative average cost across the busiest PIXL GPU passes.");

			PIXLUI::SectionBanner(
				"PIXL GPU HOTSPOTS");
			const float maxAverage = pipelinePasses.empty() ? 1.0f : std::max(pipelinePasses.front().averageMs, 0.001f);
			const size_t visibleRows = std::min<size_t>(6, pipelinePasses.size());
			for (size_t i = 0; i < visibleRows; ++i) {
				const auto& row = pipelinePasses[i];
				drawSignalRow(row.label.c_str(), row.averageMs, maxAverage, "ms avg");
				if (ImGui::IsItemHovered()) {
					if (auto _tt = Util::HoverTooltipWrapper())
						ImGui::Text("Current %.3f ms   P95 %.3f ms", row.currentMs, row.p95Ms);
				}
			}
			if (pipelinePasses.empty())
				ImGui::TextDisabled("Waiting for stable workload samples...");
		}
		...
		if (this->settings.ShowVRAM) {
			if (auto adapter = menu->GetDXGIAdapter3()) {
				DXGI_QUERY_VIDEO_MEMORY_INFO memory{};
				if (SUCCEEDED(adapter->QueryVideoMemoryInfo(0, DXGI_MEMORY_SEGMENT_GROUP_LOCAL, &memory)) && memory.Budget > 0) {
					const float usedGB = static_cast<float>(memory.CurrentUsage) / (1024.0f * 1024.0f * 1024.0f);
					const float budgetGB = static_cast<float>(memory.Budget) / (1024.0f * 1024.0f * 1024.0f);
					const float usage = std::clamp(usedGB / budgetGB, 0.0f, 1.0f);
					PIXLUI::SectionBanner("VIDEO MEMORY");
					char vramLabel[96]{};
					std::snprintf(vramLabel, sizeof(vramLabel), "%.2f / %.2f GB", usedGB, budgetGB);
					ImGui::TextDisabled("%s", vramLabel);
					ImGui::ProgressBar(usage, { -1.0f, 7.0f * scale }, "");
				}
			}
		}

		if (menu->GetSettings().DeveloperMode && this->settings.ShowCSPasses && !pipelinePasses.empty()) {
			if (ImGui::CollapsingHeader("GPU PASS DETAIL", ImGuiTreeNodeFlags_DefaultOpen)) {
				const size_t detailRows = std::min<size_t>(12, pipelinePasses.size());
				for (size_t i = 0; i < detailRows; ++i) {
					const auto& row = pipelinePasses[i];
					ImGui::TextColored(PIXLUI::ToVec4(PIXLUI::Colors::TextMuted), "%02zu", i + 1);
					ImGui::SameLine(42.0f * scale);
					ImGui::TextUnformatted(row.label.c_str());
					ImGui::SameLine(std::max(ImGui::GetCursorPosX() + 8.0f * scale, ImGui::GetWindowWidth() - 170.0f * scale));
					ImGui::TextDisabled("%.3f / %.3f ms", row.averageMs, row.p95Ms);
				}
			}
		}

		if (menu->GetSettings().DeveloperMode) {
...
void PulseProfiler::DrawFPS()
{
	...
	if (this->settings.ShowPreFGFrameTimeGraph) {
		...
		// ProfilerStyleScope owns the PIXL graph signal colour.
	...
	if (this->settings.ShowPostFGFrameTimeGraph && this->state.isFrameGenerationActive) {
		...
		bool isFrameGenActive = globals::pipeline::imageReconstruction.IsFrameGenerationActive();
...
void PulseProfiler::DrawPostFGFrameTimeGraph()
{
	// Prepare overlay text
	char overlay_text[128];
	snprintf(overlay_text, IM_ARRAYSIZE(overlay_text),
...
ABTestAggregator& PulseProfiler::GetABTestAggregator()
{
...
/**
  ...
  */
void PulseProfiler::DrawABTestResultsTable()
{
	auto* abTestingManager = ABTestingManager::GetSingleton();
...
/**
  ...
  */
void PulseProfiler::DrawABTestStatisticalValidity(const Menu::ThemeSettings& theme, const ABTestAggregator& aggregator) const
{
...
/**
  ...
  */
void PulseProfiler::ConvertABTestResultsToRows(const std::vector<AggregatedDrawCallStats>& results, std::vector<DrawCallRow>& mainRows, std::vector<DrawCallRow>& summaryRows) const
{
	mainRows.clear();
	summaryRows.clear();
...
/**
  ...
  */
ABTestLegends PulseProfiler::BuildABTestLegends(const Menu::ThemeSettings& theme) const
{
...
/**
  ...
  */
std::vector<ColumnConfig> PulseProfiler::BuildABTestResultsTableColumns(const Menu::ThemeSettings& theme, const ABTestLegends& legends) const
{
	std::vector<ColumnConfig> columns = {
		...
		{ legends.delta.header,
			[theme, legends](const DrawCallRow& row, int) {
				...
				ImGui::PushStyleColor(ImGuiCol_Text, color);
				ImGui::Text("%s", Util::FormatDeltaWithPercent(row.frameTime, *row.testFrameTime, PulseProfiler::Settings::kPercentDisplayThreshold).c_str());
				ImGui::PopStyleColor();
				if (ImGui::IsItemHovered()) {
					if (auto _tt = Util::HoverTooltipWrapper()) {
						if (row.testFrameTime.has_value()) {
...
/**
  ...
  */
void PulseProfiler::DrawABTestSection(const std::vector<DrawCallRow>& allRows)
{
	auto* menu = Menu::GetSingleton();
	auto* abTestingManager = ABTestingManager::GetSingleton();
...
void PulseProfiler::DrawDrawCallsTable(const std::vector<DrawCallRow>& mainRows, const std::vector<DrawCallRow>& summaryRows)
{
	static bool clearTestDataRequested = false;
	auto& overlay = globals::pipeline::pulseProfiler;
	auto* menu = Menu::GetSingleton();
	const auto& theme = menu->GetTheme();
...
DrawCallLegends PulseProfiler::BuildDrawCallLegends(const Menu::ThemeSettings& theme, bool anyTestData) const
{
	(void)anyTestData;
...
std::vector<ColumnConfig> PulseProfiler::BuildDrawCallTableColumns(const Menu::ThemeSettings& theme, const DrawCallLegends& legends, bool anyTestData)
{
	...
	columns.push_back(ColumnConfig{
		...
		MakeMetricColumn(theme, [](const DrawCallRow& row) { return row.frameTime; }, [](const auto& theme, float value, const DrawCallRow& row) {
			...
			return Util::GetThresholdColor(value, PulseProfiler::Settings::kFrameTimeGoodThreshold, PulseProfiler::Settings::kFrameTimeWarningThreshold, theme.StatusPalette.SuccessColor, theme.StatusPalette.Warning, theme.StatusPalette.Error); }, [](float /*value*/, const DrawCallRow& row) { return Util::FormatMilliseconds(row.frameTime) + " (" + Util::FormatPercent(row.percent) + ")"; }, legends.frameTime.tooltip), [](const DrawCallRow& a, const DrawCallRow& b, bool asc) { return asc ? (a.percent < b.percent) : (a.percent > b.percent); }, [legends]() {
			 if (ImGui::IsItemHovered()) {
				 if (auto _tt = Util::HoverTooltipWrapper()) {
	...
	columns.push_back(ColumnConfig{
		...
		MakeMetricColumn(theme, [](const DrawCallRow& row) { return row.costPerCall; }, [](const auto& theme, float value, const DrawCallRow& row) {
			...
			return Util::GetThresholdColor(value, PulseProfiler::Settings::kCostPerCallGoodThreshold, PulseProfiler::Settings::kCostPerCallWarningThreshold, theme.StatusPalette.SuccessColor, theme.StatusPalette.Warning, theme.StatusPalette.Error); }, [](float value, const DrawCallRow&) { return (value < PulseProfiler::Settings::kMicrosecondThreshold && value > 0.0f) ? Util::FormatMicroseconds(value * 1000.0f) : Util::FormatMilliseconds(value); }, legends.costPerCall.tooltip), [](const DrawCallRow& a, const DrawCallRow& b, bool asc) { return asc ? (a.costPerCall < b.costPerCall) : (a.costPerCall > b.costPerCall); }, [legends]() {
			 if (ImGui::IsItemHovered()) {
				 if (auto _tt = Util::HoverTooltipWrapper()) {
	...
	if (anyTestData) {
		...
		columns.push_back(ColumnConfig{
			...
			MakeMetricColumn(theme, [](const DrawCallRow& row) { return row.testCostPerCall; }, [](const auto& theme, float value, const DrawCallRow& row) {
					 ...
					 if (value > row.costPerCall)
						 return theme.StatusPalette.Error;
					 return theme.Palette.Text; }, [](float value, const DrawCallRow&) { return (value < PulseProfiler::Settings::kMicrosecondThreshold && value > 0.0f) ? Util::FormatMicroseconds(value * 1000.0f) : Util::FormatMilliseconds(value); }, legends.testCostPerCall.tooltip), [](const DrawCallRow& a, const DrawCallRow& b, bool asc) {
				 float aVal = a.testCostPerCall.value_or(FLT_MAX);
				 float bVal = b.testCostPerCall.value_or(FLT_MAX);
				 return asc ? (aVal < bVal) : (aVal > bVal); }, [legends]() {
				 if (ImGui::IsItemHovered()) {
...
std::pair<std::vector<DrawCallRow>, std::vector<DrawCallRow>> PulseProfiler::BuildDrawCallRows() const
{
	...
	DrawCallRow csPassesRow = {
		...
		0.0f,
		std::string("GPU time spent in PIXL Renderer compute passes (profiled)."),
...
/**
  ...
  */
std::function<void(int, int, const DrawCallRow&)> PulseProfiler::CreateTableRowHandler(const std::vector<ColumnConfig>& columns)
{
...
/**
  ...
  */
void PulseProfiler::HandleShaderToggle(const DrawCallRow& row, bool wasEnabled)
{
	auto maybeType = magic_enum::enum_cast<RE::BSShader::Type>(row.shaderType);
	if (!maybeType.has_value()) {
		return;
...
/**
  ...
  */
void PulseProfiler::HandleTotalRowToggle()
{
...
/**
  ...
  */
void PulseProfiler::UpdateShaderTestData(int shaderType, float frameTime, float costPerCall)
{
...
/**
  ...
  */
void PulseProfiler::UpdateAllShaderTestData()
{
...
std::string PulseProfiler::GetTestDataTooltip() const
{
...
void PulseProfiler::CaptureTestData()
{
	auto* abTestingManager = ABTestingManager::GetSingleton();
...
void PulseProfiler::ClearTestData()
{
	testData.clear();
	testDataSource = TestDataSource::None;
}
...
void PulseProfiler::UpdateShaderTestDataEntry(int shaderType, float frameTime, float costPerCall, float percent)
{
	testData[shaderType] = { frameTime, costPerCall, percent };
}

void PulseProfiler::UpdateSummaryTestData(float smoothedFrameTime, float otherFrameTime, float otherPercent, float totalCostPerCall)
{
...
void PulseProfiler::UpdateGraphValues()
{
	// Check if Frame Generation is active
	state.isFrameGenerationActive = globals::pipeline::imageReconstruction.IsFrameGenerationActive();
	...
	if (state.isFrameGenerationActive) {
		...
		float fgDeltaTime = globals::pipeline::imageReconstruction.GetFrameGenerationFrameTime();
