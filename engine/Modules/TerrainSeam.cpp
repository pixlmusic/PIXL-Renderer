...
#define I18N_KEY_PREFIX "feature.terrain_seam."

NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(
	TerrainSeam::Settings,
	Enabled)

void TerrainSeam::DrawSettings()
{
	Util::UIntCheckbox(T(TKEY("enable"), "Enable Terrain Seam"), &settings.Enabled);
...
void TerrainSeam::LoadSettings(json& o_json)
{
	settings = o_json;
}

void TerrainSeam::SaveSettings(json& o_json)
{
	o_json = settings;
}

ID3D11VertexShader* TerrainSeam::GetTerrainVertexShader()
{
...
ID3D11VertexShader* TerrainSeam::GetTerrainOffsetVertexShader()
{
...
ID3D11ComputeShader* TerrainSeam::GetDepthBlendShader()
{
	if (!depthBlendShader) {
		logger::debug("Compiling DepthBlend.hlsl");
		depthBlendShader = (ID3D11ComputeShader*)Util::CompileShader(L"Data\\Shaders\\TerrainSeam\\DepthBlend.hlsl", {}, "cs_5_0");
	}
	return depthBlendShader;
}

void TerrainSeam::SetupResources()
{
...

	{
		...
		Util::SetResourceName(terrainDepth.texture, "TerrainSeam::TerrainDepth");
		...
		Util::SetResourceName(terrainDepth.depthSRV, "TerrainSeam::TerrainDepth SRV");
		...
		Util::SetResourceName(terrainDepth.views[0], "TerrainSeam::TerrainDepth DSV");
	}

	{
		...
		blendedDepthTexture = new Texture2D(texDesc, "TerrainSeam::BlendedDepth");
		...
		blendedDepthTexture16 = new Texture2D(texDesc, "TerrainSeam::BlendedDepth16");
...

	{
		...
		Util::SetResourceName(terrainDepthStencilState, "TerrainSeam::DepthStencilState");
	}
}

void TerrainSeam::PostPostLoad()
{
	Hooks::Install();
}
...
void TerrainSeam::ResetDepth()
{
	TracyD3D11Zone(globals::state->tracyCtx, "Terrain Seam - Reset Depth");
	auto context = globals::d3d::context;
...
void TerrainSeam::ResetTerrainDepth()
{
	TracyD3D11Zone(globals::state->tracyCtx, "Terrain Seam - Reset Terrain Depth");
	if (globals::state->frameAnnotations)
		globals::state->BeginPerfEvent("Terrain Seam - Reset Terrain Depth");
...
void TerrainSeam::BlendPrepassDepths()
{
	ZoneScoped;
	TracyD3D11Zone(globals::state->tracyCtx, "Terrain Seam - Blend Prepass Depths");
	if (globals::state->frameAnnotations)
		globals::state->BeginPerfEvent("Terrain Seam - Blend Prepass Depths");
	...
	{
		TracyD3D11Zone(globals::state->tracyCtx, "Terrain Seam - Depth Blend CS");
		...
		globals::profiler->BeginPass("TerrainSeam::DepthBlend");
...
void TerrainSeam::ClearShaderCache()
{
	if (terrainVertexShader) {
...
void TerrainSeam::Hooks::Main_RenderDepth::thunk(bool a1, bool a2)
{
	ZoneScopedS(8);

	auto& singleton = globals::pipeline::terrainSeam;
...
void TerrainSeam::Hooks::BSBatchRenderer__RenderPassImmediately::thunk(RE::BSRenderPass* a_pass, uint32_t a_technique, bool a_alphaTest, uint32_t a_renderFlags)
{
	auto& singleton = globals::pipeline::terrainSeam;
	auto shaderCache = globals::shaderCache;
	...
	if (shaderCache->IsEnabled() && singleton.settings.Enabled) {
		if (singleton.renderDepth) {
			...
			if (inTerrain && a_pass->geometry) {
				// GroundResponse's raised shell can render out to 2048 units. Keep
				// TerrainSeam's alternate terrain-depth capture coherent across the
				// same range; the old 1024-unit cutoff produced a moving boundary
				// as landscape bounds crossed the capture radius.
				constexpr float terrainDepthCaptureDistance = 2048.0f;
				if ((a_pass->geometry->worldBound.center.GetDistance(singleton.eyePosition) - a_pass->geometry->worldBound.radius) > terrainDepthCaptureDistance) {
					inTerrain = false;
				}
			}
...
void TerrainSeam::RenderTerrainSeamPasses()
{
	ZoneScoped;
	...
	if (terrainPassCount != 0 || noBlendPassCount != 0) {
		TracyD3D11Zone(globals::state->tracyCtx, "Terrain Seam - Render Passes");
		if (globals::state->frameAnnotations)
			globals::state->BeginPerfEvent("Terrain Seam - Render Passes");
		...
		for (auto& renderPass : terrainRenderPasses) {
			if (groundResponse.loaded)
				groundResponse.PrepareTerrainPass(renderPass.a_pass);
			...
			if (groundResponse.loaded)
				groundResponse.FinishTerrainPass();
		}

		// Prevent the final landscape pass's classification/b13 payload from
		// leaking into subsequent no-blend meshes or later Lighting draws.
		if (groundResponse.loaded)
			groundResponse.BindDefaultRuntimeData();
