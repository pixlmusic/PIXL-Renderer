...
void FidelityFX::Present(bool a_useFrameGeneration, bool a_isHDR)
{
	...
	auto renderSize = float2{ (float)globals::game::graphicsState->screenWidth, (float)globals::game::graphicsState->screenHeight } * imageReconstruction.resolutionScale;
	...
	if (a_useFrameGeneration) {
		...
		dispatchParameters.jitterOffset.x = -imageReconstruction.jitter.x;
		dispatchParameters.jitterOffset.y = -imageReconstruction.jitter.y;
