...
#define I18N_KEY_PREFIX "feature.tissue_diffusion."

NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(TissueDiffusion::DiffusionProfile,
	BlurRadius, Thickness, Strength, Falloff)

NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(
	TissueDiffusion::Settings,
...
void TissueDiffusion::DrawSettings()
{
	if (ImGui::TreeNodeEx(T(TKEY("settings"), "Settings"), ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (settings.SSMode == 0) {
			...
			Util::UIntSlider(T(TKEY("burley_samples"), "Burley Samples"), &settings.BurleySamples, 1, 64, "%u", ImGuiSliderFlags_AlwaysClamp);
			if (auto _tt = Util::HoverTooltipWrapper())
				ImGui::TextWrapped("Stable low-discrepancy Burley samples. Higher values reduce spatial noise at a proportional compute cost; no restart is required.");
			if (ImGui::TreeNodeEx(T(TKEY("base_profile"), "Base Profile"), ImGuiTreeNodeFlags_DefaultOpen)) {
...
float3 TissueDiffusion::Gaussian(DiffusionProfile& a_profile, float variance, float r)
{
	/**
...
float3 TissueDiffusion::Profile(DiffusionProfile& a_profile, float r)
{
	/**
...
void TissueDiffusion::CalculateKernel(DiffusionProfile& a_profile, Kernel& kernel)
{
...
void TissueDiffusion::DrawSSS()
{
...

	{
		...
		{
			TracyD3D11Zone(globals::state->tracyCtx, "Tissue Diffusion - Prepass");
		...
		if (settings.SSMode == 0) {
			...
			{
				TracyD3D11Zone(globals::state->tracyCtx, "Tissue Diffusion - Horizontal");
				...
				globals::profiler->BeginPass("TissueDiffusion::HorizontalBlur");
				context->Dispatch(dispatchCount.x, dispatchCount.y, 1);
			...
			{
				TracyD3D11Zone(globals::state->tracyCtx, "Tissue Diffusion - Vertical");
				...
				globals::profiler->BeginPass("TissueDiffusion::VerticalBlur");
				context->Dispatch(dispatchCount.x, dispatchCount.y, 1);
				globals::profiler->EndPass();
			}
		} else if (settings.SSMode == 1) {
			...
			{
				TracyD3D11Zone(globals::state->tracyCtx, "Tissue Diffusion - Burley");
				...
				globals::profiler->BeginPass("TissueDiffusion::Burley");
				context->Dispatch(dispatchCount.x, dispatchCount.y, 1);
				globals::profiler->EndPass();
			}
		}
...
void TissueDiffusion::SetupResources()
{
	{
...
void TissueDiffusion::RestoreDefaultSettings()
{
	settings = {};
}

void TissueDiffusion::LoadSettings(json& o_json)
{
	settings = o_json;
	settings.ScatterMode = std::clamp(settings.ScatterMode, (int)kPreScatter, (int)kPreAndPostScatter);
}

void TissueDiffusion::SaveSettings(json& o_json)
{
	o_json = settings;
}

void TissueDiffusion::ClearShaderCache()
{
	if (prepassSS) {
		prepassSS->Release();
		prepassSS = nullptr;
...
ID3D11ComputeShader* TissueDiffusion::GetComputeShaderPrepass()
{
	if (!prepassSS) {
		logger::debug("Compiling prepassSS");
		prepassSS = (ID3D11ComputeShader*)Util::CompileShader(L"Data\\Shaders\\TissueDiffusion\\DiffuseExtractionCS.hlsl", {}, "cs_5_0");
	}
	return prepassSS;
}

ID3D11ComputeShader* TissueDiffusion::GetComputeShaderHorizontalBlur()
{
	if (!horizontalSSBlur) {
		logger::debug("Compiling horizontalSSBlur");
		horizontalSSBlur = (ID3D11ComputeShader*)Util::CompileShader(L"Data\\Shaders\\TissueDiffusion\\SeparableSSSCS.hlsl", { { "HORIZONTAL", "" } }, "cs_5_0");
	}
	return horizontalSSBlur;
}

ID3D11ComputeShader* TissueDiffusion::GetComputeShaderVerticalBlur()
{
	if (!verticalSSBlur) {
		logger::debug("Compiling verticalSSBlur");
		verticalSSBlur = (ID3D11ComputeShader*)Util::CompileShader(L"Data\\Shaders\\TissueDiffusion\\SeparableSSSCS.hlsl", {}, "cs_5_0");
	}
	return verticalSSBlur;
}

ID3D11ComputeShader* TissueDiffusion::GetComputeShaderBurley()
{
	if (!burleySS) {
		logger::debug("Compiling burleySS");
		burleySS = (ID3D11ComputeShader*)Util::CompileShader(L"Data\\Shaders\\TissueDiffusion\\SeparableSSSCS.hlsl", { { "BURLEY", "" } }, "cs_5_0");
	}
	return burleySS;
}
...
void TissueDiffusion::BSLightingShader_SetupSkin(RE::BSRenderPass* a_pass)
{
...
void TissueDiffusion::Hooks::BSLightingShader_SetupGeometry::thunk(RE::BSShader* This, RE::BSRenderPass* Pass, uint32_t RenderFlags)
{
	globals::pipeline::tissueDiffusion.BSLightingShader_SetupSkin(Pass);
