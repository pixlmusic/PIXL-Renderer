...
struct ThinSurface final : RenderModule
{
	virtual inline std::string GetName() override { return "Thin Surface"; }
	virtual std::string GetDisplayName() override { return T("feature.thin_surface.name", "Thin Surface"); }
	virtual inline std::string GetShortName() override { return "ThinSurface"; }
	virtual inline std::string_view GetShaderDefineName() override { return "THIN_SURFACE"sv; }
	virtual inline std::string_view GetCategory() const override { return ModuleGroups::kMaterials; }
	virtual std::pair<std::string, std::vector<std::string>> GetModuleSummary() override
	{
		return { T("feature.thin_surface.description", "Thin Surface provides realistic rendering of thin fabric and other translucent materials.\nThis feature supports multiple material models for different types of translucent surfaces."),
			{ T("feature.thin_surface.key_feature_1", "Multiple translucency material models (rim edge, isotropic/anisotropic fabric)"),
				T("feature.thin_surface.key_feature_2", "Realistic fabric translucency with directional light transmission"),
				T("feature.thin_surface.key_feature_3", "Per-material override support via NIF extra data"),
				T("feature.thin_surface.key_feature_4", "Configurable transparency and softness controls"),
				T("feature.thin_surface.key_feature_5", "Performance-optimized translucency calculations") } };
	}
