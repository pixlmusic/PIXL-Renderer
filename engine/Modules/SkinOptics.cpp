...
void SkinOptics::DrawSettings()
{
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.controls_microscopic_roughness_of_stratum_corneum_layer", "Controls microscopic roughness of stratum corneum layer"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.secondary_roughness", "Secondary Roughness"), &settings.SkinSecondRoughness, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.smoothness_of_epidermal_cell_layer_reflections", "Smoothness of epidermal cell layer reflections"));
		ImGui::BulletText(T("feature.skin_optics.should_be_30_50_lower_than_primary", "Should be 30-50%% lower than Primary"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.specular_texture_multiplier", "Specular Texture Multiplier"), &settings.SkinSpecularTexMultiplier, 0.0f, 10.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.multiplier_for_specular_map", "Multiplier for specular map"));
		ImGui::BulletText("%s", T("feature.skin_optics.a_multiplier_for_the_vanilla_specular_map_applied", "A multiplier for the vanilla specular map, applied to the first layer's roughness"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.secondary_specular_strength", "Secondary Specular Strength"), &settings.SecondarySpecularStrength, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.intensity_of_secondary_specular_highlights", "Intensity of secondary specular highlights"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.fresnel_f0", "Fresnel F0"), &settings.F0, 0.0f, 0.1f, "%.4f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.fresnel_reflectance", "Fresnel reflectance"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.base_color_multiplier", "Base Color Multiplier"), &settings.BaseColorMultiplier, 0.0f, 2.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.multiplier_for_the_base_color_texture", "Multiplier for the base color texture"));
	}

	ImGui::Spacing();
	ImGui::Text("%s", T("feature.skin_optics.options_for_additional_roughness_and_specular_maps", "Options for additional roughness and specular maps."));

	ImGui::SliderFloat(T("feature.skin_optics.physical_main_roughness_multiplier", "Physical Main Roughness Multiplier"), &settings.PhysicalMainRoughnessMultiplier, 0.0f, 2.0f, "%.2f");
	ImGui::SliderFloat(T("feature.skin_optics.physical_second_roughness_multiplier", "Physical Second Roughness Multiplier"), &settings.PhysicalSecondRoughnessMultiplier, 0.0f, 2.0f, "%.2f");
	ImGui::SliderFloat(T("feature.skin_optics.physical_specular_multiplier", "Physical Specular Multiplier"), &settings.PhysicalSpecularStrength, 0.0f, 2.0f, "%.2f");

	ImGui::Spacing();

	ImGui::SliderFloat(T("feature.skin_optics.extra_edge_roughness", "Extra Edge Roughness"), &settings.ExtraEdgeRoughness, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.extra_roughness_at_the_edges_of_the_skin", "Extra roughness at the edges of the skin, to approximate peach fuzz on the face."));
	}

	ImGui::SliderFloat(T("feature.skin_optics.fuzz_strength", "Fuzz Strength"), &settings.FuzzStrength, 0.0f, 2.0f, "%.2f");

	ImGui::SliderFloat(T("feature.skin_optics.fuzz_roughness", "Fuzz Roughness"), &settings.FuzzRoughness, 0.1f, 1.0f, "%.2f");

	ImGui::SliderFloat(T("feature.skin_optics.fuzz_f0", "Fuzz F0"), &settings.FuzzF0, 0.0f, 0.5f, "%.4f");
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.translucency_of_the_sss_transmittance_effect", "Translucency of the SSS Transmittance effect"));
	}

	ImGui::SliderFloat(T("feature.skin_optics.sss_width", "SSS Width"), &settings.sssWidth, 0.0f, 1.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
	...
	ImGui::SliderFloat(T("feature.skin_optics.wetness_fade_out_time", "Wetness Fade Out Time"), &settings.WetFadeTime, 0.0f, 50.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.how_many_seconds_it_takes_for_skin_to", "How many seconds it takes for skin to fully dry after leaving water. Higher values mean wetness lingers longer."));
	}
	...
	if (!settings.UseDynamicWetness) {
		...
		if (auto _tt = Util::HoverTooltipWrapper()) {
			ImGui::Text(T("feature.skin_optics.the_character_starts_sweating_when_their_stamina_drops", "The character starts sweating when their stamina drops below this percentage. For example, 0.75 means sweat appears below 75%% stamina."));
		}
	...
	ImGui::SliderFloat(T("feature.skin_optics.wetness_perlin_noise_scale", "Wetness Perlin Noise Scale"), &settings.WetParams.x, 0.0f, 1024.0f, "%1.f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.controls_the_size_of_the_wet_dry_pattern", "Controls the size of the wet/dry pattern on skin. Higher values create a finer, more detailed pattern; lower values produce larger, broader wet patches."));
	}
	ImGui::SliderFloat(T("feature.skin_optics.wetness_perlin_noise_lacunarity", "Wetness Perlin Noise Lacunarity"), &settings.WetParams.y, 0.0f, 2.0f, "%.1f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.controls_how_much_fine_detail_is_added_to", "Controls how much fine detail is added to the wetness pattern. Higher values add more small-scale variation on top of the base pattern."));
	}
	ImGui::SliderFloat(T("feature.skin_optics.wetness_perlin_noise_persistence", "Wetness Perlin Noise Persistence"), &settings.WetParams.z, 0.0f, 20.0f, "%.2f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.controls_the_overall_contrast_and_roughness_of_the", "Controls the overall contrast and roughness of the wetness pattern. Higher values make the pattern more pronounced and varied."));
	}
	ImGui::SliderFloat(T("feature.skin_optics.wetness_normal_scale", "Wetness Normal Scale"), &settings.WetParams.w, 0.0f, 20.0f, "%.1f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::Text("%s", T("feature.skin_optics.controls_how_bumpy_wet_skin_appears_higher_values", "Controls how bumpy wet skin appears. Higher values create more visible surface ripples and distortion on wet areas."));
	}
	...
	ImGui::SliderFloat(T("feature.skin_optics.body_tiling_multiplier", "Body Tiling Multiplier"), &settings.BodyTilingMultiplier, 0.5f, 5.0f, "%.1f");
	if (auto _tt = Util::HoverTooltipWrapper()) {
...
void SkinOptics::LoadSkinDetailTexture()
{
	...
	try {
		std::filesystem::path path{ "Data\\Shaders\\SkinOptics\\skin_detail_n.dds" };
...
void SkinOptics::SetupResources()
{
...
void SkinOptics::ReloadSkinDetail()
{
	logger::debug("Reloading skin detail texture...");
	LoadSkinDetailTexture();
}

void SkinOptics::Prepass()
{
...
struct SKIN_BSLightingShader_SetupMaterial
{
	static void thunk(RE::BSLightingShader* shader, RE::BSLightingShaderMaterialBase const* material)
	{
		...
		auto& skin = globals::pipeline::skinOptics;
		if (skin.loaded) {
...
void SkinOptics::PostPostLoad()
{
	logger::info("[Advanced SkinOptics] Hooking BSLightingShader::SetupMaterial");
	stl::write_vfunc<0x4, SKIN_BSLightingShader_SetupMaterial>(RE::VTABLE_BSLightingShader[0]);
	Hooks::Install();
}

SkinOptics::SkinOpticsData SkinOptics::GetCommonBufferData()
{
	SkinOpticsData data{};
...
void SkinOptics::LoadSettings(json& o_json)
{
	settings = o_json;
}

void SkinOptics::SaveSettings(json& o_json)
{
	o_json = settings;
}

void SkinOptics::RestoreDefaultSettings()
{
	settings = {};
}
...
float SkinOptics::GetWaterHeight(const RE::TESObjectREFR* a_ref, const RE::NiPoint3& a_pos)
{
...
float4 SkinOptics::GetWetness(RE::BSGeometry* geometry)
{
...
void SkinOptics::SetupExtraTexture(RE::BSLightingShaderMaterialBase const* material, RE::BSTextureSet* inTextureSet, uint32_t i_hashKey)
{
	if (!inTextureSet || material->normalTexture == nullptr) {
		logger::error("[Advanced SkinOptics] SetupExtraTexture : Texture set is null for material: {}", i_hashKey);
		return;
	}
	...
	if (hashKey == 0 || hashKey != i_hashKey) {
		logger::error("[Advanced SkinOptics] SetupExtraTexture : Invalid hash key for material: {}", i_hashKey);
		return;
	}
	...
	if (auto normalPath = inTextureSet->GetTexturePath(RE::BSTextureSet::Texture::kNormal)) {
		...
		logger::error("[Advanced SkinOptics] SetupExtraTexture : No specular or normal texture found in texture set from material: {}", hashKey);
	...
	logger::debug("[Advanced SkinOptics] SetupExtraTexture : Extra texture path: {} for {}", extraTexturePath, foundPath ? foundPath : "(none)");
	logger::debug("[Advanced SkinOptics] SetupExtraTexture : Wetness texture path: {} for {}", wetnessTexturePath, foundPath ? foundPath : "(none)");
	...
	if (workingExtraPtr.hasExtraTexture || workingExtraPtr.hasWetnessTexture) {
		logger::debug("[Advanced SkinOptics] SetupExtraTexture : Extra texture set with hash key: {}", hashKey);
	} else {
		logger::debug("[Advanced SkinOptics] SetupExtraTexture : Failed to set extra texture for material: {}", hashKey);
	}
}

void SkinOptics::BSLightingShader_SetupMaterial(RE::BSLightingShaderMaterialBase const* material)
{
	...
	hashKey = material->hashKey;
	if (hashKey == 0) {
		logger::error("[Advanced SkinOptics] BSLightingShader_SetupMaterial : Invalid hash key for material: {}", static_cast<int>(materialFeature));
		return;
	}

	if (!skinExtraTextures.contains(hashKey)) {
		// logger::debug("[Advanced SkinOptics] BSLightingShader_SetupMaterial : Setting up extra texture for material: {}", static_cast<int>(materialFeature));
		globals::pipeline::skinOptics.SetupExtraTexture(material, materialTextureSet, hashKey);
	}
...
void SkinOptics::BSLightingShader_SetupGeometry(RE::BSRenderPass* a_pass)
{
	auto context = globals::d3d::context;
...
void SkinOptics::SetShaderResources(ID3D11DeviceContext* a_context)
{
...
void SkinOptics::Hooks::BSLightingShader_SetupGeometry::thunk(RE::BSShader* This, RE::BSRenderPass* Pass, uint32_t RenderFlags)
{
	auto& skin = globals::pipeline::skinOptics;
