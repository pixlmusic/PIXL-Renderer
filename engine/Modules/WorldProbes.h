...
/**
	 * RenderModule that generates dynamic cube maps for environment mapping and reflections.
	 * 
	 ...
	 */
	struct WorldProbes : RenderModule
{
public:
	const std::string defaultDynamicCubeMapSavePath = "Data\\textures\\WorldProbes";
	...
	virtual inline std::string GetName() override { return "World Probes"; }
	virtual std::string GetDisplayName() override { return T("feature.world_probes.name", "World Probes"); }
	virtual inline std::string GetShortName() override { return "WorldProbes"; }
	virtual inline std::string_view GetShaderDefineName() override { return "WORLD_PROBES"; }
	...
	{
		return { T("feature.world_probes.description", "Provides real-time environment mapping and reflections by generating dynamic cube maps that capture the surrounding environment, enabling realistic reflections on surfaces."),
			{ T("feature.world_probes.key_feature_1", "Real-time environment capture for realistic reflections"),
				T("feature.world_probes.key_feature_2", "Dynamic cube map generation based on camera position"),
				T("feature.world_probes.key_feature_3", "Enhanced water reflections with environmental details"),
				T("feature.world_probes.key_feature_4", "Optimized cubemap inference and irradiance calculation") } };
	};
