...
struct TerrainSeam : RenderModule
{
private:
	static constexpr std::string_view MOD_ID = "157076";

public:
	virtual inline std::string GetName() override { return "Terrain Seam"; }
	virtual std::string GetDisplayName() override { return T("feature.terrain_seam.name", "Terrain Seam"); }
	/** @brief Returns the short identifier name. */
	virtual inline std::string GetShortName() override { return "TerrainSeam"; }
	virtual inline std::string GetModuleSupportLink() override { return MakeNexusModURL(MOD_ID); }
	virtual inline std::string_view GetShaderDefineName() override { return "TERRAIN_SEAM"; }
	...
	{
		return { T("feature.terrain_seam.description", "Provides seamless blending between terrain and objects, eliminating harsh transitions where objects meet the ground for more natural-looking landscapes."),
			{ T("feature.terrain_seam.key_feature_1", "Seamless terrain-to-object blending transitions"),
				T("feature.terrain_seam.key_feature_2", "Advanced depth buffer manipulation for smooth integration"),
				T("feature.terrain_seam.key_feature_3", "Support for alternative terrain rendering modes"),
				T("feature.terrain_seam.key_feature_4", "Multi-pass rendering optimization for complex scenes"),
				T("feature.terrain_seam.key_feature_5", "Enhanced visual continuity in landscape interactions") } };
	};
	...
	/** @brief Draws the ImGui settings panel for Terrain Seam configuration. */
	...
	struct Hooks
	{
		...
		static void Install()
		{
			...
			logger::info("[Terrain Seam] Installed hooks");
		}
	};
};
