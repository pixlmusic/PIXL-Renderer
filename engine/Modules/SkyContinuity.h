...
struct SkyContinuity : RenderModule
{
private:
	static constexpr std::string_view MOD_ID = "153543";

public:
	virtual inline std::string GetName() override { return "Sky Continuity"; }
	virtual std::string GetDisplayName() override { return T("feature.sky_continuity.name", "Sky Continuity"); }
	virtual inline std::string GetShortName() override { return "SkyContinuity"; }
	...
	{
		return { T("feature.sky_continuity.description", "Synchronizes volumetric lighting and shadows with the actual sun and moon positions in the sky."),
			{ T("feature.sky_continuity.key_feature_1", "Fixes the mismatch between the positions of the sun and moons and the lighting direction"),
				T("feature.sky_continuity.key_feature_2", "Includes a configurable alternative sun path for more realistic and dramatic lighting"),
				T("feature.sky_continuity.key_feature_3", "Smoothly switches the light source between the sun and moons based on visibility"),
				T("feature.sky_continuity.key_feature_4", "Moon light source can be switched between Masser, Secunda, or the brightest"),
				T("feature.sky_continuity.key_feature_5", "Automatic calculation of moon lighting intensity based on moon phase"),
				T("feature.sky_continuity.key_feature_6", "Fixes the sun appearing higher on the horizon when the player gains altitude") } };
	};
	...
	/** @brief Draws the ImGui settings panel for Sky Continuity configuration. */
