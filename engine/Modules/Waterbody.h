...
#include "Waterbody/Flowmap.h"
#include "Waterbody/WaterCache.h"

#include <atomic>

/** @brief Replaces distant water tiles with LOD0 water to eliminate water LOD mismatch. */
struct Waterbody : OverlayFeature
{
	virtual inline std::string GetName() override { return "Waterbody"; }
	virtual std::string GetDisplayName() override { return T("feature.waterbody.name", "Waterbody"); }
	/** @brief Returns the short identifier used for file paths and logging. */
	virtual inline std::string GetShortName() override { return "Waterbody"; }
	virtual inline std::string_view GetShaderDefineName() override { return "WATERBODY"; }
	...
	{
		return { T("feature.waterbody.description", "Waterbody provides a comprehensive fix to water LOD mismatch by replacing distant water tiles with LOD0 (Close Water)."),
			{ T("feature.waterbody.key_feature_1", "Unifies distant and close water appearance, streamlining all lighting visuals."),
				T("feature.waterbody.key_feature_2", "Completely and fundamentally resolves water LOD mismatch issues."),
				T("feature.waterbody.key_feature_3", "Provides background systems for water geometry rendering, allowing more advanced water effects."),
				T("feature.waterbody.key_feature_4", "Improves vanilla performance by using optimized water meshes for distant water.") } };
	};
