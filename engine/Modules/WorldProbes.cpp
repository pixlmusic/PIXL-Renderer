...
#define I18N_KEY_PREFIX "feature.world_probes."

constexpr auto MIPLEVELS = 8;

NLOHMANN_DEFINE_TYPE_NON_INTRUSIVE_WITH_DEFAULT(
	WorldProbes::Settings,
	EnabledSSR,
	EnabledCreator);

std::vector<std::pair<std::string_view, std::string_view>> WorldProbes::GetShaderDefineOptions()
{
...
void WorldProbes::DrawSettings()
{
	...
	if (ImGui::TreeNodeEx(T(TKEY("dynamic_cubemap_creator"), "Dynamic Cubemap Creator"), ImGuiTreeNodeFlags_DefaultOpen)) {
		...
		if (auto _tt = Util::HoverTooltipWrapper())
			ImGui::TextWrapped("Enables the cubemap authoring controls in this page. The CREATOR shader define must also be present; adding that define requires a shader rebuild.");
...
void WorldProbes::LoadSettings(json& o_json)
{
	settings = o_json;
	recompileFlag = true;
}

void WorldProbes::SaveSettings(json& o_json)
{
	o_json = settings;
}

void WorldProbes::RestoreDefaultSettings()
{
	settings = {};
	recompileFlag = true;
}

void WorldProbes::DataLoaded()
{
	MenuOpenCloseEventHandler::Register();
}

void WorldProbes::PostPostLoad()
{
}
...
void WorldProbes::ClearShaderCache()
{
	if (updateCubemapCS) {
...
ID3D11ComputeShader* WorldProbes::GetComputeShaderUpdate()
{
	if (!updateCubemapCS) {
		logger::debug("Compiling UpdateCubemapCS");
		updateCubemapCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\UpdateCubemapCS.hlsl", {}, "cs_5_0"));
	}
	return updateCubemapCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderUpdateReflections()
{
	if (!updateCubemapReflectionsCS) {
		logger::debug("Compiling UpdateCubemapCS REFLECTIONS");
		updateCubemapReflectionsCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\UpdateCubemapCS.hlsl", { { "REFLECTIONS", "" } }, "cs_5_0"));
	}
	return updateCubemapReflectionsCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderUpdateFakeReflections()
{
	if (!updateCubemapFakeReflectionsCS) {
		logger::debug("Compiling UpdateCubemapCS FAKEREFLECTIONS");
		updateCubemapFakeReflectionsCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\UpdateCubemapCS.hlsl", { { "FAKEREFLECTIONS", "" } }, "cs_5_0"));
	}
	return updateCubemapFakeReflectionsCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderInferrence()
{
	if (!inferCubemapCS) {
		logger::debug("Compiling InferCubemapCS");
		inferCubemapCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\InferCubemapCS.hlsl", {}, "cs_5_0"));
	}
	return inferCubemapCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderInferrenceReflections()
{
	if (!inferCubemapReflectionsCS) {
		logger::debug("Compiling InferCubemapCS REFLECTIONS");
		inferCubemapReflectionsCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\InferCubemapCS.hlsl", { { "REFLECTIONS", "" } }, "cs_5_0"));
	}
	return inferCubemapReflectionsCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderInferrenceFakeReflections()
{
	if (!inferCubemapFakeReflectionsCS) {
		logger::debug("Compiling InferCubemapCS FAKEREFLECTIONS");
		inferCubemapFakeReflectionsCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\InferCubemapCS.hlsl", { { "FAKEREFLECTIONS", "" } }, "cs_5_0"));
	}
	return inferCubemapFakeReflectionsCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderSpecularIrradiance()
{
	if (!specularIrradianceCS) {
		logger::debug("Compiling SpecularIrradianceCS");
		specularIrradianceCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\SpecularIrradianceCS.hlsl", {}, "cs_5_0"));
	}
	return specularIrradianceCS;
}

ID3D11ComputeShader* WorldProbes::GetComputeShaderBC6HEncode()
{
	if (!bc6hEncodeCS) {
		logger::debug("Compiling BC6HEncodeCS");
		bc6hEncodeCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\WorldProbes\\BC6HEncodeCS.hlsl", {}, "cs_5_0"));
	}
	return bc6hEncodeCS;
}

void WorldProbes::UpdateCubemapCapture(bool a_reflections)
{
	...
	globals::profiler->BeginPass(a_reflections ? "WorldProbes::CaptureReflections" : "WorldProbes::Capture");
 ...
 */
void WorldProbes::Inferrence(bool a_reflections)
{
	auto renderer = globals::game::renderer;
	auto context = globals::d3d::context;
	...
	globals::profiler->BeginPass(a_reflections ? "WorldProbes::InferReflections" : "WorldProbes::Infer");
	context->Dispatch((uint32_t)std::ceil(envCaptureTexture->desc.Width / 8.0f), (uint32_t)std::ceil(envCaptureTexture->desc.Height / 8.0f), 6);
	globals::profiler->EndPass();
 ...
 */
void WorldProbes::Irradiance(bool a_reflections, uint32_t a_startLevel, uint32_t a_endLevel, bool a_doSetup)
{
	auto context = globals::d3d::context;
	...
	// Compute pre-filtered specular environment map for the requested mip range.
	{
		...
		const auto passName = a_reflections
			? std::format("WorldProbes::IrradianceReflections{}", suffix)
			: std::format("WorldProbes::Irradiance{}", suffix);
...
void WorldProbes::CompressToBC6H(bool a_reflections)
{
	auto context = globals::d3d::context;
	...
	globals::profiler->BeginPass(a_reflections ? "WorldProbes::BC6HReflections" : "WorldProbes::BC6H");
 ...
 */
void WorldProbes::UpdateCubemap()
{
	ZoneScoped;
	...
	if (recompileFlag) {
		logger::debug("Recompiling for World Probes");
...
void WorldProbes::SetupResources()
{
...

	{
		...
		// Texture2DArray SRVs used by BC6H encoder (Load() requires array dimension, not TextureCube)
		{
			...
			Util::SetResourceName(envTextureArraySRV, "WorldProbes::EnvTexture ArraySRV");
			...
			Util::SetResourceName(envReflectionsTextureArraySRV, "WorldProbes::EnvReflections ArraySRV");
		}

		envInferredTexture = new Texture2D(texDesc, "WorldProbes::EnvInferred");
		...
		// Mip count must match the BC6H target so block-equivalent dimensions align.
		{
			...
			bc6hScratchTexture = new Texture2D(scratchDesc, "WorldProbes::BC6HScratch");
			...
			for (std::uint32_t level = 0; level < bc6hMipLevels; ++level) {
				...
				Util::SetResourceName(bc6hScratchUAVs[level], "WorldProbes::BC6HScratch UAV mip%u", level);
			}
		}
		...
		// BC6H compressed cubemap textures (shader-read-only).
		{
			...
			envTextureBC6H = new Texture2D(bc6hDesc, "WorldProbes::EnvTextureBC6H");
			envTextureBC6H->CreateSRV(bc6hSRVDesc);

			envReflectionsTextureBC6H = new Texture2D(bc6hDesc, "WorldProbes::EnvReflectionsBC6H");
			envReflectionsTextureBC6H->CreateSRV(bc6hSRVDesc);
		}

		updateCubemapCB = new ConstantBuffer(ConstantBufferDesc<UpdateCubemapCB>(), "WorldProbes::UpdateCubemapCB");
	}

	{
		bc6hEncodeCB = new ConstantBuffer(ConstantBufferDesc<BC6HEncodeCB>(), "WorldProbes::BC6HEncodeCB");
	}

	{
		spmapCB = new ConstantBuffer(ConstantBufferDesc<SpecularMapFilterSettingsCB>(), "WorldProbes::SpmapCB");
	}

	{
		...
		for (std::uint32_t level = 1; level < MIPLEVELS; ++level) {
			...
			Util::SetResourceName(uavArray[level - 1], "WorldProbes::EnvTexture UAV mip%u", level);
		}

		for (std::uint32_t level = 1; level < MIPLEVELS; ++level) {
			uavDesc.Texture2DArray.MipSlice = level;
			DX::ThrowIfFailed(device->CreateUnorderedAccessView(envReflectionsTexture->resource.get(), &uavDesc, &uavReflectionsArray[level - 1]));
			Util::SetResourceName(uavReflectionsArray[level - 1], "WorldProbes::EnvReflections UAV mip%u", level);
		}
	}

	{
		DirectX::CreateDDSTextureFromFile(device, L"Data\\Shaders\\WorldProbes\\defaultcubemap.dds", nullptr, &defaultCubemap);
	}
}

void WorldProbes::Reset()
{
