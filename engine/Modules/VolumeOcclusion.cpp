...
void VolumeOcclusion::SetupResources()
{
	...
	// Create samplers
	{
		...
		Util::SetResourceName(linearSampler, "VolumeOcclusion::LinearSampler");
	}

	// Compile compute shaders
	std::vector<std::pair<const char*, const char*>> defines;
	defines.push_back({ "DOWNSAMPLE_SHADOW_MIP0", nullptr });
	downsampleShadowMip0CS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\VolumeOcclusion\\DownsampleShadowCS.hlsl", defines, "cs_5_0"));
	defines.clear();
	defines.push_back({ "DOWNSAMPLE_SHADOW_MIP1", nullptr });
	downsampleShadowMip1CS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\VolumeOcclusion\\DownsampleShadowCS.hlsl", defines, "cs_5_0"));

	defines.clear();
	defines.push_back({ "BLUR_HORIZONTAL", nullptr });
	blurShadowHorizontalCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\VolumeOcclusion\\BlurShadowCS.hlsl", defines, "cs_5_0"));
	defines.clear();
	defines.push_back({ "BLUR_VERTICAL", nullptr });
	blurShadowVerticalCS = static_cast<ID3D11ComputeShader*>(Util::CompileShader(L"Data\\Shaders\\VolumeOcclusion\\BlurShadowCS.hlsl", defines, "cs_5_0"));
}

void VolumeOcclusion::ClearShaderCache()
{
...
void VolumeOcclusion::CopyShadowLightData()
{
	ZoneScoped;
...

	{
		...
		if (shadowView) {
			...
			if (!shadowCopyTexture) {
				...
				Util::SetResourceName(shadowCopyTexture, "VolumeOcclusion::ShadowCopy");
				...
				Util::SetResourceName(shadowCopySRV, "VolumeOcclusion::ShadowCopy SRV");
				...
				Util::SetResourceName(shadowCopyMip0SRV, "VolumeOcclusion::ShadowCopy SRV mip0");
				...
				Util::SetResourceName(shadowCopyMip1SRV, "VolumeOcclusion::ShadowCopy SRV mip1");
				...
				Util::SetResourceName(shadowCopyMip0UAV, "VolumeOcclusion::ShadowCopy UAV mip0");
				...
				Util::SetResourceName(shadowCopyMip1UAV, "VolumeOcclusion::ShadowCopy UAV mip1");
				...
				Util::SetResourceName(shadowBlurTempTexture, "VolumeOcclusion::ShadowBlurTemp");
				...
				Util::SetResourceName(shadowBlurTempMip0SRV, "VolumeOcclusion::ShadowBlurTemp SRV mip0");

				srvDesc.Texture2D.MostDetailedMip = 1;
				srvDesc.Texture2D.MipLevels = 1;
				DX::ThrowIfFailed(device->CreateShaderResourceView(shadowBlurTempTexture, &srvDesc, &shadowBlurTempMip1SRV));
				Util::SetResourceName(shadowBlurTempMip1SRV, "VolumeOcclusion::ShadowBlurTemp SRV mip1");

				uavDesc.Texture2D.MipSlice = 0;
				DX::ThrowIfFailed(device->CreateUnorderedAccessView(shadowBlurTempTexture, &uavDesc, &shadowBlurTempMip0UAV));
				Util::SetResourceName(shadowBlurTempMip0UAV, "VolumeOcclusion::ShadowBlurTemp UAV mip0");

				uavDesc.Texture2D.MipSlice = 1;
				DX::ThrowIfFailed(device->CreateUnorderedAccessView(shadowBlurTempTexture, &uavDesc, &shadowBlurTempMip1UAV));
				Util::SetResourceName(shadowBlurTempMip1UAV, "VolumeOcclusion::ShadowBlurTemp UAV mip1");
			}
			...
			if (shadowResource) {
				...
				if (shadowTexture) {
					...
					globals::profiler->BeginPass("VolumeOcclusion::DownsampleMip0");
					...
					globals::profiler->BeginPass("VolumeOcclusion::DownsampleMip1");
					context->Dispatch(dispatchSize, dispatchSize, 1);
					globals::profiler->EndPass();
					...
					// 11x11 separable blur for Mip 0
					{
						...
						globals::profiler->BeginPass("VolumeOcclusion::BlurHMip0");
						...
						globals::profiler->BeginPass("VolumeOcclusion::BlurVMip0");
					...
					// 11x11 separable blur for Mip 1
					{
						...
						context->CSSetUnorderedAccessViews(0, 1, csUavs, nullptr);
						context->CSSetShader(blurShadowHorizontalCS, nullptr, 0);
						globals::profiler->BeginPass("VolumeOcclusion::BlurHMip1");
						...
						context->CSSetUnorderedAccessViews(0, 1, csUavs, nullptr);
						context->CSSetShader(blurShadowVerticalCS, nullptr, 0);
						globals::profiler->BeginPass("VolumeOcclusion::BlurVMip1");
...
void VolumeOcclusion::SetSharedShadowMapSRV(ID3D11DeviceContext* a_context, ID3D11ShaderResourceView* a_srv)
{
...
void VolumeOcclusion::LoadSettings(json&)
{
	// No settings currently
}

void VolumeOcclusion::SaveSettings(json&)
{
	// No settings currently
}

void VolumeOcclusion::RestoreDefaultSettings()
{
	// No settings currently
}
...
void VolumeOcclusion::PostPostLoad()
{
...
bool VolumeOcclusion::HasShaderDefine(RE::BSShader::Type)
{
	return true;
}
