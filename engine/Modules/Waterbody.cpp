...
namespace
{
	...
	{
		std::error_code ec;
		logger::error("[Waterbody] {} load failed: {}, loose file present: {}", dataRelativePath, magic_enum::enum_name(error),
	...
	void PatchBranchToUnconditional(const std::uintptr_t address, const char* label)
	{
		...
		if (IsShortBranch(bytes[0])) {
			...
			logger::debug("[Waterbody] Patched short branch for {} at {:X}", label, address);
			return;
		}
		...
		if (IsNearConditionalBranch(bytes[0], bytes[1])) {
			...
			logger::debug("[Waterbody] Patched near branch for {} at {:X}", label, address);
			return;
		}

		logger::error("[Waterbody] Skipping {} patch at {:X}: unexpected branch bytes {:02X} {:02X}", label, address, bytes[0], bytes[1]);
	}
	...
	/** @brief Disables the vanilla LOD water and flow map paths that Waterbody supersedes. Irreversible, so it must only run once the meshes are validated. */
	bool DisableVanillaWaterLOD()
	{
		...
		if (!CanPatchBranch(attachedMeshAddLoop) || !CanPatchBranch(lodWaterAddLoop)) {
			logger::error("[Waterbody] Unexpected branch bytes at {:X} or {:X}; another mod may patch the same code", attachedMeshAddLoop, lodWaterAddLoop);
			return false;
		}
		patched = true;
...
void Waterbody::LoadSettings(json& o_json)
{
	settings = o_json;
}

void Waterbody::SaveSettings(json& o_json)
{
	o_json = settings;
}

void Waterbody::RestoreDefaultSettings()
{
	settings = {};
}

void Waterbody::DrawSettings()
{
...
void Waterbody::DrawOverlay()
{
	...
	if (waterCache->IsBuildRunning()) {
		...
		ImGui::TextColored(themeSettings.StatusPalette.Error, T("feature.waterbody.error_water_cache_generation_failed_for_worldspaces_check", "ERROR: Water cache generation failed for %d WorldSpaces. Check installation and PIXLRenderer.log"), snapshot.failed);

		ImGui::End();
	}
}

bool Waterbody::IsOverlayVisible() const
{
	return true;
}

void Waterbody::DataLoaded()
{
	...
	const auto fail = [this](std::string reason) {
		logger::error("[Waterbody] {}; distant water falls back to vanilla LOD", reason);
	...
	logger::debug("[Waterbody] Water mesh loaded");
	...
	logger::debug("[Waterbody] Optimised water mesh loaded");
	...
	if (!MenuOpenCloseEventHandler::Register()) {
		logger::warn("[Waterbody] MenuOpenCloseEventHandler registration failed");
	}
}

RE::BSEventNotifyControl Waterbody::MenuOpenCloseEventHandler::ProcessEvent(const RE::MenuOpenCloseEvent* event, RE::BSTEventSource<RE::MenuOpenCloseEvent>*)
{
	if (!event)
		return RE::BSEventNotifyControl::kContinue;

	auto& singleton = globals::pipeline::waterbody;
...
bool Waterbody::MenuOpenCloseEventHandler::Register()
{
	...
	if (!ui) {
		logger::error("[Waterbody] UI event source not found");
		return false;
	}

	const auto source = ui->GetEventSource<RE::MenuOpenCloseEvent>();
	if (!source) {
		logger::error("[Waterbody] MenuOpenCloseEvent source not found");
		return false;
	}

	source->AddEventSink(&singleton);
	registered = true;
	logger::info("[Waterbody] Registered MenuOpenCloseEventHandler");
	return true;
}
	...
	bool ReadHashFile(const std::filesystem::path& path, uint64_t& outHash)
	{
		for (int attempt = 0; attempt < kShareRetries; ++attempt) {
			...
			if (handle) {
				...
				if (!ok)
					logger::warn("[Waterbody] '{}' exists but could not be fully read; treating as no persisted hash", path.string());
				return ok;
			}
			...
			logger::warn("[Waterbody] Failed to open '{}' for reading (error {})", path.string(), err);
			return false;
		}
		return false;
	}
	...
	bool WriteHashFile(const std::filesystem::path& path, uint64_t hash)
	{
		for (int attempt = 0; attempt < kShareRetries; ++attempt) {
			...
			if (handle) {
				...
				if (!ok)
					logger::error("[Waterbody] Failed to persist load-order hash to '{}'; cache will regenerate again next launch", path.string());
				return ok;
			}
			const DWORD err = GetLastError();
			if ((err == ERROR_SHARING_VIOLATION || err == ERROR_LOCK_VIOLATION) && attempt + 1 < kShareRetries) {
			...
			logger::error("[Waterbody] Failed to open '{}' for writing (error {}); cache will regenerate again next launch", path.string(), err);
			return false;
		}
		return false;
	}
...
bool Waterbody::LoadOrderChanged()
{
	...
	const std::filesystem::path path = Util::PathHelpers::GetWaterbodyCachePath() / "LoadOrder.hash";

	uint64_t existingHash = 0;
	ReadHashFile(path, existingHash);

	const bool changed = hash != existingHash;
	logger::debug("[Waterbody] Load order hash: computed={:#x} persisted={:#x} changed={}", hash, existingHash, changed);
...
void Waterbody::SetFlowmapTex() const
{
	...
	if (!gFlowMapSourceTex || !gFlowMapSize) {
		logger::error("[Waterbody] Global pointers not initialized");
		return;
	}
	...
	*gFlowMapSize = flowmap->GetWidth();
	...
	logger::debug("[Waterbody] [Flowmap] Texture set");
}

void Waterbody::PostPostLoad()
{
	...
	logger::info("[Waterbody] Installed hooks");
}

void Waterbody::TESWaterSystem_InitializeWater_SetWaterShaderMaterialParams::thunk(RE::TESWaterForm* form, RE::BSWaterShaderMaterial* material)
{
...
int32_t Waterbody::BSWaterShaderMaterial_ComputeCRC32::thunk(RE::BSWaterShaderMaterial* material, uint32_t srcHash)
{
...
bool Waterbody::IsWaterDataReady() const
{
	return waterCache && waterMesh && optimisedWaterMesh;
}

bool Waterbody::IsExteriorWorldspaceActive() const
{
...
void Waterbody::UpdateWaterLODCull() const
{
...
void Waterbody::TES_SetWorldSpace::thunk(RE::TES* tes, RE::TESWorldSpace* worldSpace, bool isExterior)
{
...
void Waterbody::TES_DestroySkyCell::thunk(RE::TES* tes)
{
	func(tes);
...
void Waterbody::BGSTerrainNode_UpdateWaterMeshSubVisibility::thunk(const RE::BGSTerrainNode* node, RE::BSMultiBoundNode* waterParent)
{
	if (!globals::pipeline::waterbody.IsWaterDataReady()) {
...
void Waterbody::BGSTerrainBlock_Attach::thunk(RE::BGSTerrainBlock* block)
{
	...
	if (singleton.IsWaterDataReady() && block && block->loaded && !block->attached && block->chunk && block->water) {
		...
		if (!instructions) {
			logger::warn("[Waterbody] No instructions found for {} chunk at {}, {}", worldSpace->GetFormEditorID(), node->baseCellX, node->baseCellY);
...
void Waterbody::BGSTerrainBlock_Detach::thunk(RE::BGSTerrainBlock* block)
{
	if (!block) {
		return;
	}
	...
	if (wasWaterAttached) {
		...
		if (auto waterLOD = globals::pipeline::waterbody.gWaterLOD; waterLOD && *waterLOD)
		...
		block->waterAttached = false;
		globals::pipeline::waterbody.UpdateWaterLODCull();
	}
}

void Waterbody::BSWaterShader_SetupGeometry::thunk(RE::BSShader* waterShader, RE::BSRenderPass* pass)
{
	const auto& singleton = globals::pipeline::waterbody;
...
void Waterbody::TESWaterSystem_UpdateDisplacementMeshPosition::thunk(RE::TESWaterSystem* waterSystem)
{
	func(waterSystem);
