...
struct TerrainOcclusion : public RenderModule
{
public:
	virtual inline std::string GetName() override { return "Terrain Occlusion"; }
	virtual std::string GetDisplayName() override { return T("feature.terrain_occlusion.name", "Terrain Occlusion"); }
	/** @brief Returns the short identifier name. */
	virtual inline std::string GetShortName() override { return "TerrainOcclusion"; }
	virtual inline std::string_view GetShaderDefineName() override { return "TERRAIN_OCCLUSION"; }
	...
	{
		return { T("feature.terrain_occlusion.description", "Adds realistic shadow casting from terrain features using heightmap data to create accurate terrain shadows that enhance depth perception and visual realism."),
			{ T("feature.terrain_occlusion.key_feature_1", "Heightmap-based terrain shadow calculation"),
				T("feature.terrain_occlusion.key_feature_2", "Dynamic shadow updates based on sun position"),
				T("feature.terrain_occlusion.key_feature_3", "Support for custom heightmap files"),
				T("feature.terrain_occlusion.key_feature_4", "Real-time shadow preprocessing and computation"),
				T("feature.terrain_occlusion.key_feature_5", "Integration with existing shadow systems") } };
	};
	...
	/** @brief Draws the ImGui settings panel for Terrain Occlusion configuration. */
