...
void SkyContinuity::DrawSettings()
{
	...
	if (auto _tt = Util::HoverTooltipWrapper()) {
		ImGui::TextUnformatted(T(TKEY("enabled_tooltip"), "Enable or disable Sky Continuity features."));
	}
	...
	ImGui::Checkbox(T(TKEY("fade_volumetric_lighting"), "Fade Light Volumes"), &settings.DimLightVolumes);
	if (auto _tt = Util::HoverTooltipWrapper()) {
	...
	if (settings.DimSunlightUnderHorizon || settings.DimLightVolumes) {
...
void SkyContinuity::LoadSettings(json& o_json)
{
	settings = o_json;
...
void SkyContinuity::SaveSettings(json& o_json)
{
	o_json = settings;
}

void SkyContinuity::RestoreDefaultSettings()
{
	settings = {};
	SetSunAngle();
}

void SkyContinuity::PostPostLoad()
{
	moonAndStarsLoaded = GetModuleHandle(L"po3_MoonMod.dll");
	if (moonAndStarsLoaded)
		logger::info("[Sky Continuity] Moon and Stars detected, compatibility enabled");
	...
	logger::info("[Sky Continuity] Installed hooks");
}

void SkyContinuity::DataLoaded()
{
...
void SkyContinuity::DisableOnConflict(std::string_view conflictName)
{
	failedLoadedMessage = fmt::format("Disabled as {} has been detected, both cannot be used together", conflictName);
	loaded = false;
	settings.Enabled = false;
	logger::warn("[Sky Continuity] {}", failedLoadedMessage);
}

void SkyContinuity::OnSkyUpdateColors(RE::Sky* sky)
{
	...
	if (gVolumetricLighting) {
		...
		if (settings.DimLightVolumes)
			vlFactor *= currentDim;
		gVolumetricLighting->intensity *= vlFactor;
	}
}

void SkyContinuity::Sky_Update::thunk(RE::Sky* sky)
{
	func(sky);
	globals::pipeline::skyContinuity.Update(sky);
}
...
void SkyContinuity::SetSunAngle()
{
...
void SkyContinuity::SetSkyRotation(const RE::Sky* sky, RE::TESObjectCELL* cell)
{
...
void SkyContinuity::ProcessSun(const RE::Sky* sky, RE::NiPoint3 dirs[], float intensities[])
{
	const auto sun = sky->sun;
	RE::NiPoint3 dir;
	float dist;
...
void SkyContinuity::ProcessMoon(const RE::Sky* sky, const Caster type, RE::NiPoint3 dirs[], float intensities[])
{
...
inline void SkyContinuity::CalculateSunDirectionAndDistance(const RE::Sun* sun, RE::NiPoint3& outDir, float& outDistance)
{
...
inline void SkyContinuity::CalculateAlternateSunDirectionAndDistance(RE::NiPoint3& outDir, float& outDist, const float time, const float sunrise, const float sunset, const float sunAngle)
{
...
inline void SkyContinuity::SetSunPosition(const RE::Sun* sun, const RE::NiPoint3& dir, const float distance)
{
...
void SkyContinuity::ShadowFader::Reset()
{
	target = Caster::Sun;
...
void SkyContinuity::ShadowFader::Update(const RE::Sky* sky, RE::NiPoint3 dirs[], float intensities[], float fadeDuration, float fadeAdvance)
{
	auto isValidDir = [](const RE::NiPoint3& d) { return d.x != 0.0f || d.y != 0.0f || d.z != 0.0f; };

	Caster best;

	if (globals::pipeline::skyContinuity.sunBelowHorizon) {
...
void SkyContinuity::ShadowFader::LockSunElevation(RE::NiPoint3 dirs[])
{
	...
	const auto& skyContinuity = globals::pipeline::skyContinuity;
	const int sunIdx = static_cast<int>(Caster::Sun);
	const float minElev = DirectX::XMConvertToRadians(skyContinuity.settings.MinShadowElevation);
	if (skyContinuity.sunSetting) {
		if (skyContinuity.currentDim <= SunsetHeadingLockThreshold) {
	...
	} else if (skyContinuity.sunRising) {
		if (!sunriseReleased) {
...
void SkyContinuity::ShadowFader::SetLighting(const RE::Sky* sky, RE::NiPoint3 dir)
{
	ClampDirection(dir);
...
inline void SkyContinuity::ShadowFader::SetDirection(RE::NiPoint3& dir, float headingRadians, float elevRadians)
{
...
inline void SkyContinuity::ShadowFader::SetElevation(RE::NiPoint3& dir, float elevRadians)
{
	SetDirection(dir, std::atan2(dir.y, dir.x), elevRadians);
}

float SkyContinuity::ShadowFader::ComputeVLFactor(const RE::NiPoint3& current, const RE::NiPoint3& target)
{
...
inline void SkyContinuity::ShadowFader::ClampDirection(RE::NiPoint3& dir)
{
	const float minDegrees = globals::pipeline::skyContinuity.settings.MinShadowElevation;
