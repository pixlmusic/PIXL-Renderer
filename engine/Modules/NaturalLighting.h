...
struct NaturalLighting : RenderModule
{
public:
	virtual inline std::string GetName() override { return "Natural Lighting"; }
	virtual std::string GetDisplayName() override { return T("feature.natural_lighting.name", "Natural Lighting"); }

	/** @brief Returns the short identifier used for file paths and logging. */
	virtual inline std::string GetShortName() override { return "NaturalLighting"; }

	virtual inline std::string_view GetShaderDefineName() override { return "NATURAL_LIGHTING"; }
	...
	{
		return { T("feature.natural_lighting.description", "Implements an additional inverse square falloff for lighting which allows for a more physically accurate and realistic looking light attenuation."),
			{ T("feature.natural_lighting.key_feature_1", "Automatic light radius calculation based on intensity"),
				T("feature.natural_lighting.key_feature_2", "Lights smoothly fade out at a configurable cutoff, solving the infinite distance problem"),
				T("feature.natural_lighting.key_feature_3", "Does not modify any existing lighting"),
				T("feature.natural_lighting.key_feature_4", "Requires the use of mods with lights enabled for inverse square falloff."),
				T("feature.natural_lighting.key_feature_5", "Full integration with Light Placer") } };
	}
	 ...
	 */
	void ProcessLight(RadiantGrid::LightData& light, RE::BSLight* bsLight, RE::NiLight* niLight) const;
