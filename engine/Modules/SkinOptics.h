...
/** @brief PIXL skin rendering with dual specular lobes, detail textures, and wetness response. */
struct SkinOptics : RenderModule
{
	static SkinOptics* GetSingleton()
	{
		static SkinOptics singleton;
		return &singleton;
	}

	virtual inline std::string GetName() override { return "Skin Optics"; }
	virtual inline std::string GetDisplayName() override { return T("feature.skin_optics.name", "Skin Optics"); }
	virtual inline std::string GetShortName() override { return "SkinOptics"; }
	virtual inline std::string_view GetShaderDefineName() override { return "PIXL_SKIN"; }
	...
	{
		return {
			T("feature.skin_optics.description", "Skin Optics gives characters a natural layered skin response across dry, wet, indoor and outdoor lighting."),
			{ T("feature.skin_optics.key_feature_1", "Physically-based dual specular lobes for realistic skin highlights"),
				T("feature.skin_optics.key_feature_2", "Tiled skin detail textures for enhanced realism"),
				T("feature.skin_optics.key_feature_3", "Extra texture support for roughness, translucency, and wetness"),
				T("feature.skin_optics.key_feature_4", "Reworked wetness system for dynamic skin effects") }
		};
	}
	...
	virtual bool AffectsCachedShader(RE::BSShader::Type t, std::uint32_t descriptor, CachedShaderStage stage) override
	{
		...
		const bool skinnedGeometry = (descriptor & (1u << 1u)) != 0u;
		// Skin Optics executes on FaceGen skin (4/5), hair (6), and other skinned
		// actor geometry for its per-actor dynamic-wetness response. Static-object
		// and vertex permutations cannot observe this module.
		return technique == 4u || technique == 5u || technique == 6u || skinnedGeometry;
	}


	virtual void RestoreDefaultSettings() override;
	/** @brief Draws the ImGui settings panel for Skin Optics configuration. */
	...
	struct alignas(16) SkinOpticsData
	{
		float4 skinParams;
		float4 skinParams2;
	...
	SkinOpticsData GetCommonBufferData();
	...
	struct Hooks
	{
		...
		static void Install()
		{
			...
			logger::info("[Advanced SkinOptics] Installed hooks");
			return;
		}
	};
