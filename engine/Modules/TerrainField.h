...
struct TerrainField : RenderModule
{
private:
	static constexpr std::string_view MOD_ID = "143149";

public:
	virtual inline std::string GetName() override { return "Terrain Field"; }
	virtual std::string GetDisplayName() override { return T("feature.terrain_field.name", "Terrain Field"); }
	/** @brief Returns the short identifier name. */
	virtual inline std::string GetShortName() override { return "TerrainField"; }
	virtual inline std::string_view GetShaderDefineName() override { return "TERRAIN_FIELD"; }
	...
	{
		return { T("feature.terrain_field.description", "Provides enhanced terrain material support for terrain mods that require additional texture slots and parallax mapping capabilities."),
			{ T("feature.terrain_field.key_feature_1", "Extended texture slot support for terrain materials"),
				T("feature.terrain_field.key_feature_2", "Parallax mapping integration for terrain textures"),
				T("feature.terrain_field.key_feature_3", "Automatic terrain material detection and setup"),
				T("feature.terrain_field.key_feature_4", "Support for advanced terrain modifications"),
				T("feature.terrain_field.key_feature_5", "Compatibility layer for terrain enhancement mods") } };
	};
	...
	/** @brief Installs the TESObjectLAND material setup hook early for compatibility with MaterialForge. */
	...
	virtual std::string GetModuleSupportLink() override { return MakeNexusModURL(MOD_ID); }
