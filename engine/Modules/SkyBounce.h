...
public:
	...
	virtual inline std::string GetName() override { return "SkyBounce"; }
	virtual std::string GetDisplayName() override { return T("feature.sky_bounce.name", "SkyBounce"); }
	virtual inline std::string GetShortName() override { return "SkyBounce"; }
	virtual inline std::string GetModuleSupportLink() override { return MakeNexusModURL(MOD_ID); }
	virtual inline std::string_view GetShaderDefineName() override { return "SKY_BOUNCE"; }
	...
	{
		return { T("feature.sky_bounce.description", "Simulates realistic ambient lighting by calculating sky occlusion and directional lighting, providing more accurate and natural illumination in outdoor environments."),
			{ T("feature.sky_bounce.key_feature_1", "Sky occlusion calculation for ambient lighting"),
				T("feature.sky_bounce.key_feature_2", "Directional skyBounce based on environment geometry"),
				T("feature.sky_bounce.key_feature_3", "Enhanced ambient lighting for outdoor scenes"),
				T("feature.sky_bounce.key_feature_4", "Support for varying sky illumination intensities"),
				T("feature.sky_bounce.key_feature_5", "Integration with existing lighting systems") } };
	};

	virtual bool HasShaderDefine(RE::BSShader::Type) override { return true; };

	virtual void RestoreDefaultSettings() override;
	/** @brief Draws the ImGui settings panel for SkyBounce configuration. */
	...
	static_assert(sizeof(SkyBounceCB) % 16 == 0);

	/**
	 * @brief Builds the skyBounce constant buffer data from current probe array state.
	 * @param a_inWorld Whether the player is currently in an exterior worldspace.
	 * @return Populated SkyBounceCB structure, or zeroed if not in world or map menu is open.
	 */
	SkyBounceCB GetCommonBufferData(bool a_inWorld);
	...
	// cached variables
	bool queuedResetSkyBounce = true;
	...
	/** @brief Renders the skyBounce occlusion map using the precipitation rendering system. */
