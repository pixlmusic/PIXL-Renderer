...
void State::Draw()
{
	...
	auto& terrainSeam = globals::pipeline::terrainSeam;
	auto& groundResponse = globals::pipeline::groundResponse;
	auto& terrainField = globals::pipeline::terrainField;
	auto& skyVeil = globals::pipeline::skyVeil;
	auto& skin = globals::pipeline::skinOptics;
	auto& materialForge = globals::pipeline::materialForge;
	auto context = globals::d3d::context;
	auto& volumeOcclusion = globals::pipeline::volumeOcclusion;
	...
	if (shaderCache->IsEnabled()) {
		...
		if (terrainSeam.loaded && terrainSeam.settings.Enabled) {
			ZoneScopedN("TerrainSeam::TerrainShaderHacks");
			terrainSeam.TerrainShaderHacks();
		}

		// TerrainSeam replays the real landscape pass through the normal Lighting
		// setup path. Insert GroundResponse HS/DS only after Skyrim has rebound its
		// per-draw state so the original VS/PS, materials and G-buffer remain intact.
		if (groundResponse.loaded && groundResponse.activeTerrainPass) {
			ZoneScopedN("GroundResponse::TerrainPassShaderHacks");
			groundResponse.TerrainPassShaderHacks();
		}

		if (skyVeil.loaded) {
			ZoneScopedN("SkyVeil::SkyShaderHacks");
			skyVeil.SkyShaderHacks();
		}


		if (terrainField.loaded) {
			ZoneScopedN("TerrainField::SetShaderResources");
			terrainField.SetShaderResources(context);
		}

		if (skin.loaded) {
			ZoneScopedN("SkinOptics::SetShaderResources");
			skin.SetShaderResources(context);
		}

		if (materialForge.loaded) {
			ZoneScopedN("MaterialForge::SetShaderResources");
			materialForge.SetShaderResources(context);
		}
		...
		if (currentShader && updateShader) {
			if (currentShader->shaderType.get() == RE::BSShader::Type::Utility) {
				if (currentPixelDescriptor & static_cast<uint32_t>(SIE::ShaderCache::UtilityShaderFlags::RenderShadowmask)) {
					// 13BG captures the fullscreen/depth RenderShadowmask variant only. In
					// that permutation Utility reconstructs camera-relative world position
					// from depth, so b2's ShadowMapProj[3] can be reused directly for the
					// displaced terrain receiver. Object-space shadowmask variants carry
					// per-object matrices and must never overwrite this capture.
					if (groundResponse.loaded &&
						(currentPixelDescriptor & static_cast<uint32_t>(SIE::ShaderCache::UtilityShaderFlags::RenderDepth))) {
						const bool focusShadow =
							(currentPixelDescriptor & static_cast<uint32_t>(SIE::ShaderCache::UtilityShaderFlags::LodLandscape)) != 0u;
						groundResponse.CaptureDirectionalShadowAtlas(focusShadow);
					}
					if (volumeOcclusion.loaded)
						volumeOcclusion.CopyShadowLightData();
					if (globals::pipeline::atmosphere.loaded)
						globals::pipeline::atmosphere.CaptureDirectionalShadowMap();
					if (skyBounce.loaded)
						skyBounce.CaptureShadowCascadeSRV();
				}
			}
		}

		if (globals::menu->overlayVisible && globals::pipeline::pulseProfiler.loaded && globals::pipeline::pulseProfiler.IsOverlayVisible())
			Debug();
...
void State::Reset()
{
	...
	if (auto ui = globals::game::ui) {
		...
		isLockpickingMenuOpen = ui->IsMenuOpen(RE::LockpickingMenu::MENU_NAME);
		isMapMenuOpen = ui->IsMenuOpen(RE::MapMenu::MENU_NAME);
	} else {
		isMainMenuOpen = false;
		isLoadingMenuOpen = false;
		isLockpickingMenuOpen = false;
		isMapMenuOpen = false;
	}

	// DialogueFocus is a renderer service rather than a RenderModule in v1.
	// Use real frame time rather than game-time delta: Skyrim UI/dialogue states can
	// pause or scale gameplay time, but the visual focus envelope must still settle.
	const float dialogueDelta =
		std::clamp(RE::GetSecondsSinceLastFrame(), 0.0f, 0.10f);
	DialogueFocus::GetSingleton().Update(dialogueDelta);
...
void State::Setup()
{
	...
	// DialogueFocus owns a private character-only PS constant buffer. It is kept
	// outside FeatureData b6 so no existing module offset changes.
	DialogueFocus::GetSingleton().SetupResources();
	DialogueFocus::GetSingleton().InstallHooks();
...
void State::Load(ConfigMode a_configMode, bool a_allowReload)
{
	...
	try {
		...
		constexpr std::array<std::string_view, 4> managedServices{
			"NaturalLighting", "HorizonBlend", "TerrainField", "TerrainSeam"
		};
		for (const auto service : managedServices) {
			if (disabledFeatures[std::string(service)])
				logger::info("PIXL managed service '{}' restored to enabled", service);
			disabledFeatures[std::string(service)] = false;
		}
		for (auto* feature : RenderModule::GetModuleList()) {
			try {
				const std::string featureName = feature->GetShortName();
				// Resolved here rather than in RenderModule::Load so features disabled at boot,
				// which never reach Load, still report their install state to the UI.
				std::error_code ec;
				const bool iniExists = std::filesystem::exists(Util::PathHelpers::GetModuleDescriptorPath(featureName), ec);
				...
				if (!disabledFeatures.contains(featureName) && feature->IsDisabledByDefault()) {
					...
					logger::info("RenderModule '{}' is disabled by default", featureName);
				}
				bool isDisabled = disabledFeatures.contains(featureName) && disabledFeatures[featureName];
				if (!isDisabled) {
					logger::info("Loading RenderModule: '{}'", featureName);
					...
					logger::info("RenderModule '{}' is disabled at boot.", featureName);
				}
			} catch (const std::exception& e) {
				feature->failedLoadedMessage = feature->failedLoadedMessage.empty() ?
					                                   (feature->GetName() + " failed to load. Check PIXLRenderer.log") :
		...
		PipelineHealth::ScanForOrphanedFeatureINIs();

		logger::info("Loading Settings Complete");
	} catch (const json::exception& e) {
		logger::warn("General JSON error accessing settings: {}; recreating config", e.what());
		Save(a_configMode);
		errorDetected = true;
	} catch (const std::exception& e) {
		logger::warn("General error accessing settings: {}; recreating config", e.what());
		Save(a_configMode);
		errorDetected = true;
	}
...
void State::SaveToJson(nlohmann::json& settings)
{
	...
	auto& upscalingJson = settings[imageReconstruction.GetShortName()];
	imageReconstruction.SaveSettings(upscalingJson);
...
{
	return globals::menu && globals::menu->GetSettings().DeveloperMode;
}
...
void State::CheckTypedUAVLoadSupport()
{
	...
	static const FormatEntry kFormats[] = {
		{ DXGI_FORMAT_R11G11B10_FLOAT, "R11G11B10_FLOAT", "World Probes (envCapture/Raw/Position) — non-HDR" },
		{ DXGI_FORMAT_R16G16B16A16_FLOAT, "R16G16B16A16_FLOAT", "World Probes (HDR), SkyBounce outProbeArray" },
		{ DXGI_FORMAT_R16G16B16A16_UNORM, "R16G16B16A16_UNORM", "Ground Response (collisionTexture)" },
		{ DXGI_FORMAT_R16G16_UNORM, "R16G16_UNORM", "Terrain Occlusion (RWTexShadowHeights)" },
		{ DXGI_FORMAT_R8G8B8A8_UNORM, "R8G8B8A8_UNORM", "Camera Suite UI brightness (uiTexture)" },
		{ DXGI_FORMAT_R8_UINT, "R8_UINT", "SkyBounce accumulation frames (outAccumFramesArray)" },
	...
	if (anyUnsupported) {
		logger::warn(
			...
			"Consider disabling: World Probes, Ground Response, Terrain Occlusion, SkyBounce, Camera Suite.");
	}
}
...
void State::SetupResources()
{
	...
	auto [data, size] = GetPipelineBufferData(false);
	(void)data;
...
#ifdef TRACY_ENABLE
	RenderModule::SetTracyCtx(tracyCtx);
#endif
...
{
	{
		...
		// Lighting VS already owns b12, so mirror CameraPosAdjust through the
		...
		if (imageReconstruction.loaded) {
			auto upscaleMethod = imageReconstruction.GetUpscaleMethod();
			if (temporal && upscaleMethod != ImageReconstruction::UpscaleMethod::kTAA) {
				...
				if (upscaleMethod == ImageReconstruction::UpscaleMethod::kDLSS)
					data.MipBias -= 1.0f;
			} else {
				data.MipBias = 0;
			}
		...
		if (auto sky = globals::game::sky) {
			...
			if (auto masser = sky->masser) {
				...
				if (masser->root && !masser->root->GetFlags().any(RE::NiAVObject::Flag::kHidden))
					data.MasserColor = Util::Moon::GetBlendColor(masser, Util::Moon::MasserBaseColor, globals::pipeline::skyContinuity.settings.NewMoonIntensity, globals::pipeline::skyContinuity.settings.CrescentMoonIntensity, globals::pipeline::skyContinuity.settings.FullMoonIntensity);
			}
			...
			if (auto secunda = sky->secunda) {
				...
				if (secunda->root && !secunda->root->GetFlags().any(RE::NiAVObject::Flag::kHidden))
					data.SecundaColor = Util::Moon::GetBlendColor(secunda, Util::Moon::SecundaBaseColor, globals::pipeline::skyContinuity.settings.NewMoonIntensity, globals::pipeline::skyContinuity.settings.CrescentMoonIntensity, globals::pipeline::skyContinuity.settings.FullMoonIntensity);
			}
		}
...
void State::UpdateFeatureData(bool a_inWorld)
{
	...
	auto [data, size] = GetPipelineBufferData(a_inWorld);
...
bool State::SetFeatureDisabled(const std::string& featureName, bool isDisabled)
{
	...
	if (wasPreviouslyDisabled != isDisabled) {
		...
		logger::info("RenderModule '{}' state remains: {}", featureName, isDisabled ? "Disabled" : "Enabled");
	}
...
void State::LoadTheme()
{
	...
	if (!globals::menu->LoadThemePreset(presetName)) {
		logger::warn("Failed to load preset '{}', attempting to fall back to 'PIXL'", presetName);
		if (globals::menu->LoadThemePreset("PIXL")) {
			globals::menu->GetSettings().SelectedThemePreset = "PIXL";
			logger::info("Fallback to 'PIXL' theme succeeded");
		} else {
			logger::warn("Fallback to 'PIXL' theme failed");
		}
	}
}
...
{
	return !Util::IsInterior() || globals::pipeline::interiorDaylight.IsActiveInteriorDaylight();
}
...
void State::SaveTheme()
{
	...
	logger::debug("SaveTheme() is a compatibility no-op; SelectedThemePreset is persisted by State::Save");
}
