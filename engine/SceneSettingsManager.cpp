...
// --- RenderModule Metadata (static helpers, zero coupling) ---
...
std::vector<std::string> SceneSettingsManager::GetInteriorRelevantFeatureNames()
{
	...
	auto allNames = RenderModule::GetLoadedModuleNames();
...
std::vector<std::string> SceneSettingsManager::GetFeatureSettingKeys(const std::string& featureShortName)
{
	...
	auto* feature = RenderModule::FindModuleById(featureShortName);
	if (!feature)
		return keys;
	...
	RE::BSTEventSource<RE::MenuOpenCloseEvent>*)
{
	if (a_event && a_event->menuName == RE::LoadingMenu::MENU_NAME && !a_event->opening) {
		...
		// when this event fires. Same pattern as SkyBounce::queuedResetSkyBounce.
...
{
	// Revert interior overrides on main/loading menu (same check as LinearLightCore)
...
{
	// Match SkyBounce's interior detection: sky mode != kFull
	bool interior = true;
...
void SceneSettingsManager::SaveExteriorSettings(SceneType type)
{
	...
	for (const auto& [shortName, keys] : keysToSave) {
		auto* feature = RenderModule::FindModuleById(shortName);
		if (!feature)
			continue;
...
{
	auto* feature = RenderModule::FindModuleById(entry.featureShortName);
	if (!feature)
		return;
	...
	if (verify.contains(entry.settingKey) && verify[entry.settingKey] != entry.value) {
		logger::warn("[SceneSettings] RenderModule '{}' clamped setting '{}' from {} to {}",
			entry.featureShortName, entry.settingKey,
...
void SceneSettingsManager::LoadUserSettings(SceneType type)
{
	...
	try {
		...
		for (const auto& item : data) {
			...
			if (!RenderModule::FindModuleById(entry.featureShortName))
				continue;
		...
		try {
			...
			if (featureShortName.empty()) {
				...
				if (lastUnderscore != std::string::npos) {
					...
					if (RenderModule::FindModuleById(candidate)) {
			...
			if (!RenderModule::FindModuleById(featureShortName)) {
