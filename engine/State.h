...
public:
	...
	/**
	 * @brief Strips and rewrites shader descriptor bits for PIXL Renderer' pipeline.
	...
	bool isLockpickingMenuOpen = false;
	...
	/** @brief Returns true for display-referred menus containing separately lit preview geometry. */
	bool IsDisplayReferredModelMenuOpen(RE::UI* ui) const
	{
		return IsMainOrLoadingMenuOpen(ui) || isLockpickingMenuOpen ||
		       (ui && ui->IsMenuOpen(RE::LockpickingMenu::MENU_NAME));
	}

	void UpdateSharedData(bool a_inWorld, bool a_prepass);
	/** @brief Uploads runtime feature settings immediately without rebuilding scene data. */
	...
	struct alignas(16) SharedDataCB
	{
		...
		// Reuses existing padding; SharedDataCB byte size is unchanged.
