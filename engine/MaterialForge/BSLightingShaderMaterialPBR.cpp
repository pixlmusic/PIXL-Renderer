...
void BSLightingShaderMaterialPBR::ApplyTextureSetData(const MaterialForge::PBRTextureSetData& textureSetData)
{
...
void BSLightingShaderMaterialPBR::ApplyMaterialObjectData(const MaterialForge::PBRMaterialObjectData& materialObjectData)
{
...
PhysicalMaterial::Descriptor BSLightingShaderMaterialPBR::GetPhysicalMaterialDescriptor(
	const RE::NiSourceTexture* defaultBlack,
	const RE::NiSourceTexture* defaultWhite,
	bool projectedUVActive) const
{
	PhysicalMaterial::Descriptor descriptor;
	descriptor.featureMask = pbrFlags.underlying();
	...
	setTexture(PhysicalMaterial::Texture::BaseColor, diffuseTexture);
	setTexture(PhysicalMaterial::Texture::Normal, normalTexture);
	setTexture(PhysicalMaterial::Texture::Rmaos, rmaosTexture, defaultWhite);
	setTexture(PhysicalMaterial::Texture::Emissive, emissiveTexture, defaultBlack);
	setTexture(PhysicalMaterial::Texture::Displacement, displacementTexture, defaultBlack);
	setTexture(PhysicalMaterial::Texture::Features0, featuresTexture0, defaultWhite);
	setTexture(PhysicalMaterial::Texture::Features1, featuresTexture1, defaultWhite);

	if (glintParameters.enabled) {
		descriptor.featureMask |= static_cast<std::uint32_t>(PhysicalMaterial::MaterialTrait::Glint);
	}
	if (projectedUVActive && projectedMaterialGlintParameters.enabled) {
		descriptor.featureMask |= static_cast<std::uint32_t>(PhysicalMaterial::MaterialTrait::ProjectedGlint);
	}

	descriptor.roughnessScale = GetRoughnessScale();
	descriptor.specularLevel = GetSpecularLevel();
	...
	descriptor.displacementScale = GetDisplacementScale();

	const auto& subsurfaceColor = GetSubsurfaceColor();
	descriptor.subsurfaceColor = { subsurfaceColor.red, subsurfaceColor.green, subsurfaceColor.blue };
	descriptor.subsurfaceOpacity = GetSubsurfaceOpacity();

	const auto& coatColor = GetCoatColor();
	descriptor.coatColor = { coatColor.red, coatColor.green, coatColor.blue };
	descriptor.coatStrength = GetCoatStrength();
	descriptor.coatRoughness = GetCoatRoughness();
	descriptor.coatSpecularLevel = GetCoatSpecularLevel();

	const auto& materialFuzzColor = GetFuzzColor();
	descriptor.fuzzColor = { materialFuzzColor.red, materialFuzzColor.green, materialFuzzColor.blue };
	descriptor.fuzzWeight = GetFuzzWeight();

	descriptor.projectedBaseColorScale = GetProjectedMaterialBaseColorScale();
	descriptor.projectedRoughness = GetProjectedMaterialRoughness();
	descriptor.projectedSpecularLevel = GetProjectedMaterialSpecularLevel();

	auto copyGlint = [](const GlintParameters& source, PhysicalMaterial::Glint& destination) {
		destination.screenSpaceScale = source.screenSpaceScale;
		destination.logMicrofacetDensity = source.logMicrofacetDensity;
		destination.microfacetRoughness = source.microfacetRoughness;
		destination.densityRandomization = source.densityRandomization;
	};
