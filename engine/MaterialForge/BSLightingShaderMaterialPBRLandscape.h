...
public:
	...
	bool HasGlint() const;
	/**
	 * @brief Resolves one terrain tile into the backend-neutral physical-material contract.
	 * @param textureIndex Terrain tile index in the range [0, NumTiles).
	 * @param defaultBlack Skyrim's default black texture, used to identify absent displacement.
	 * @param defaultWhite Skyrim's default white texture, used to identify absent RMAOS data.
	 * @return A versioned physical-material descriptor for the tile.
	 */
	...
	inline static std::unordered_map<BSLightingShaderMaterialPBRLandscape*, std::array<MaterialForge::PBRTextureSetData*, NumTiles>> All;


	// PIXL_GROUND_SNOW_METADATA_V2
	// Side metadata preserves Skyrim's six vanilla BSLightingShaderMaterialLandscape::
	// textureIsSnow values without moving any ABI-sensitive fields in this class.
	struct SnowMetadata
	{
		std::array<float, NumTiles> textureIsSnow{};
		bool valid = false;
	};
	inline static std::unordered_map<BSLightingShaderMaterialPBRLandscape*, SnowMetadata> SnowMetadataByMaterial;
	// members
