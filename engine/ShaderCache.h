...
namespace SIE
{
	...
	public:
		...
		RE::BSShader::Type GetShaderType() const { return shader.shaderType.get(); }
...
namespace SIE
{
	...
	public:
		...
		uint32_t pixlCompileWave = 0;                       // stage-first PIXL compile wave (0=core, 1=materials, 2=effects)
		std::mutex compilationMutex;

		/** Per-task timing record stored for post-mortem analysis and developer UI. */
		struct SlowTaskRecord
		{
			std::string key;  // ShaderCompilationTask::GetString() â€” "fxpFile:Class:defines"
			double elapsedMs = 0.0;
			double queueWaitMs = 0.0;
			int priority = 0;               // estimated compile weight (see ComputePriority)
			int defineCount = 0;            // popcount of descriptor â€” active define permutations
	...
	public:
		...
		/** @brief Returns true if the shader type is one PIXL Renderer can replace. */
		...
		void DeleteDiskCache(std::string_view a_reason = "unspecified request");
		/** @brief Deletes only the persistent family belonging to one game shader type. */
		void InvalidateDiskCache(RE::BSShader::Type a_type, std::string_view a_reason);
		...
		*/
		...
		{
			// FXC's large Lighting permutations are memory-bandwidth/codegen bound.
			// Measured on the 12 P-core target: 6 workers = 0.262 shaders/s,
			// 10 workers = 0.250 shaders/s while making the machine much less
			// responsive. Half the performance cores is the throughput sweet spot.
			const auto pCores = static_cast<int32_t>(Util::GetPerformanceCoreCount());
			return std::clamp((pCores + 1) / 2, 1, 8);
		}
