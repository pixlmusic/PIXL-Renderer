...
namespace BSShaderHooks
{
	...
	{
		logger::debug("BSShader::LoadShaders called on {} - ps count {}", bsShader->m_LoaderType, bsShader->m_PixelShaderTable.size());
		...
		if (strcmp("Lighting", bsShader->m_LoaderType) == 0) {
			...
			if (std::filesystem::exists(shaderDir)) {
				...
				for (const auto& entry : std::filesystem::directory_iterator(shaderDir)) {
					...
					logger::debug("found shader technique id {:08x} with path {}", techniqueId, entry.path().generic_string());
					foundCount++;
				...
				for (const auto& entry : bsShader->m_PixelShaderTable) {
					...
					if (tFileIt != techniqueFileMap.end()) {
						if (const auto shader = ShaderCompiler::CompileAndRegisterPixelShader(tFileIt->second)) {
							logger::debug("shader compiled successfully, replacing old shader");
							successCount++;
				...
				if (failedCount != 0)
					logger::warn("Legacy Lighting shader overrides: found={}, replaced={}, failed={}", foundCount, successCount, failedCount);
				else if (foundCount != 0)
					logger::info("Legacy Lighting shader overrides: found={}, replaced={}", foundCount, successCount);
			}
		}
		...
		if (std::filesystem::exists(shaderDir)) {
			logger::debug("Scanning shader override directory {}", shaderDir.generic_string());
			...
			if (failedCount != 0)
				logger::warn("{} shader overrides: found={}, replaced={}, failed={}", bsShader->m_LoaderType, foundCount, successCount, failedCount);
			else if (foundCount != 0)
				logger::info("{} shader overrides: found={}, replaced={}", bsShader->m_LoaderType, foundCount, successCount);
		}
	}
}
