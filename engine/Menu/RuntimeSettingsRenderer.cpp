#include "RuntimeSettingsRenderer.h"
...
void RuntimeSettingsRenderer::RenderGeneralSettings(SettingsState& state)
{
...
void RuntimeSettingsRenderer::RenderShadersTab()
{
...
void RuntimeSettingsRenderer::RenderKeybindingsTab(
	SettingsState& state)
{
...
void RuntimeSettingsRenderer::RenderInterfaceTab()
{
	...
	if (BeginTabItemWithFont(tabLabel.c_str(), Menu::FontRole::Heading)) {
		...
		if (ImGui::BeginTabBar("##tabs", ImGuiTabBarFlags_None)) {
			RenderBehaviorTab();
			// PIXL ships one authored player interface. Theme/font/style editors are
			// deliberately confined to LAB so Tune never exposes inherited skins.
...
void RuntimeSettingsRenderer::RenderBehaviorTab()
{
	...
	if (BeginTabItemWithFont(tabLabel.c_str(), Menu::FontRole::Heading)) {
...

		{
			...
			if (auto _tt = Util::HoverTooltipWrapper()) {
				ImGui::Text("%s", T("menu.settings.language_tooltip", "Select the display language for the PIXL Renderer interface."));
			}
		}
...
void RuntimeSettingsRenderer::RenderThemesTab()
{
	...
	if (BeginTabItemWithFont(tabLabel.c_str(), Menu::FontRole::Heading)) {
		...
		// Find current selection index - PIXL owns the packaged player theme.
		int currentItem = 0;
		std::string currentThemePreset = globals::menu->GetSettings().SelectedThemePreset;

		// If no theme is selected, restore the PIXL product skin.
		if (currentThemePreset.empty()) {
			currentThemePreset = "PIXL";
		...
		if (deleteThemePopup.Draw() && currentThemeInfo && !currentThemeInfo->filePath.empty()) {
			...
			if (result.success) {
				themeManager->RefreshThemes();
				globals::menu->LoadThemePreset("PIXL");
				currentThemePreset = "PIXL";
			} else {
...
void RuntimeSettingsRenderer::RenderFontsTab()
{
	...
	if (BeginTabItemWithFont(tabLabel.c_str(), Menu::FontRole::Heading)) {
		...
		if (fontCatalog.families.empty()) {
			Util::Text::Warning("%s", T("menu.settings.no_fonts_found", "No fonts found. Place .ttf files in Interface/PIXLRenderer/Fonts/"));
		}
...
void RuntimeSettingsRenderer::RenderStylingTab()
{
...
void RuntimeSettingsRenderer::RenderColorsTab()
{
	...
	if (BeginTabItemWithFont(tabLabel.c_str(), Menu::FontRole::Heading)) {
		...
		if (ImGui::TreeNodeEx(T("menu.settings.feature_headings", "RenderModule Headings"), ImGuiTreeNodeFlags_DefaultOpen)) {
