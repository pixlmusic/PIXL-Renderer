...
namespace BackgroundBlur
{
	...
	namespace
	{
		...
		// Cached SRV for non-imageReconstruction path (avoids per-frame CreateShaderResourceView)
		...
		// Camera + Post FX live preview. This is a clean copy of the currently
		// bound presentation target, captured before ImGui draws the PIXL menu.
		std::mutex livePreviewMutex;
		winrt::com_ptr<ID3D11Texture2D> livePreviewTexture;
		winrt::com_ptr<ID3D11ShaderResourceView> livePreviewSRV;
		UINT livePreviewWidth = 0;
		UINT livePreviewHeight = 0;
		DXGI_FORMAT livePreviewFormat = DXGI_FORMAT_UNKNOWN;
		double livePreviewLastCaptureTime = -1.0;
		...
		{
			return globals::pipeline::cameraSuite.ShouldUseD3D12UIBuffer();
		}
		...
		UIBufferViews GetHDRUIBufferViews(CameraSuite& hdr, ImageReconstruction& imageReconstruction)
		{
			if (imageReconstruction.d3d12SwapChainActive && ShouldUseD3D12UIBufferForBlur())
				return GetD3D12UIBufferViews(imageReconstruction.GetBlurResources());
		...
		bool ShouldSkipStartupMenuBlur(const ImageReconstruction&)
		{
			// The compilation overlay already supplies its own dimmed backdrop. More
			// importantly, startup can temporarily render ImGui without a fresh scene
			// target; blurring then feeds the previous UI frame back into itself and
			// produces repeated windows/cursor trails.
	...
	void Cleanup()
	{
...

		{
			std::lock_guard<std::mutex>
				previewLock(livePreviewMutex);
			livePreviewSRV = nullptr;
			livePreviewTexture = nullptr;
			livePreviewWidth = 0;
			livePreviewHeight = 0;
			livePreviewFormat =
				DXGI_FORMAT_UNKNOWN;
			livePreviewLastCaptureTime =
				-1.0;
		}
	...
	LivePreviewFrame CaptureLivePreview(bool effectsEnabled)
	{
		...
		if (!device ||
			...
			!camera.hdrDataCB ||
			!camera.hdrTexture ||
			!camera.hdrTexture->resource ||
			!camera.outputTexture ||
			!camera.outputTexture->resource ||
			!camera.outputTexture->srv ||
			(globals::shaderCache &&
		...
		const bool captureDue =
			!livePreviewSRV ||
			livePreviewLastCaptureTime < 0.0 ||
			now -
				livePreviewLastCaptureTime >=
				(1.0 / 30.0);
		...
		// Use CameraSuite's clean scene-only path. This snapshot is taken from
		// hdrTexture before menu blur/UI compositing, so PIXL's ImGui can never
		// recursively appear inside the live preview.
		camera.SnapshotCleanScene();

		if (!camera.cleanSceneCapture ||
			!camera.cleanSceneCapture->srv ||
			!camera.cleanSceneCapture->resource) {
			frame.srv =
				livePreviewSRV.get();
			frame.width =
				livePreviewWidth;
		...
		if (!effectsEnabled) {
			{
				...
				// Preview-only comparison. Preserve the display transform but
				// neutralize CameraSuite finishing for this one clean compose.
				camera.settings.enablePhysicalCamera =
					true;
				camera.settings.cameraInfluence =
					0.0f;
				...
				camera.settings.enableBloom =
					false;
				camera.settings.enableStormglass =
					false;
				camera.settings.enableSubmergedOptics =
					false;
				camera.settings.experimentalBodycam =
					false;
			}
		...
		ID3D11Texture2D*
			composedTexture =
				camera.ComposeCleanCapture(
					camera.cleanSceneCapture
						->srv.get(),
					true);
		...
		if (needsCreate) {
			...
			Util::SetResourceName(
				newTexture.get(),
				"BackgroundBlur::CleanLivePreview");
			Util::SetResourceName(
				newSRV.get(),
				"BackgroundBlur::CleanLivePreview SRV");
	...
	void RenderBackgroundBlur()
	{
		...
		// Check if imageReconstruction with D3D12 swap chain is active
		...
		if (ShouldSkipStartupMenuBlur(imageReconstruction))
			return;

		        // PIXL_C2_SKIP_UNSTABLE_COMPILER_BLUR
        // The startup compiler can render before the game/menu backbuffer is a
        // stable blur source. Sampling it here recursively captures PIXL's own
        // prior UI frames and creates repeated/dragging window ghosts.
        if (globals::shaderCache && globals::shaderCache->IsCompiling())
            return;
		...
		if (hdrActive) {
			...
			uiBuffer = GetHDRUIBufferViews(*hdr, imageReconstruction);
		} else if (useUpscalingBackbuffer) {
			// When D3D12 swap chain is active, get all resources in one call
			auto res = imageReconstruction.GetBlurResources();
			...
			// Cache SRV for non-imageReconstruction path (avoids CreateShaderResourceView every frame)
		...
		for (int i = 0; i < ctx->Windows.Size; i++) {
			...
			// Skip Pulse Profiler window (no blur)
			if (window->Name && std::string_view(window->Name) == "Pulse Profiler") {
				continue;
			}
			...
			// Pass UI buffer SRV/RTV for compositing and clearing during imageReconstruction gameplay
