...
void ThemeManager::SetupImGuiStyle(const Menu& menu)
{
	...
	// Safety check: If theme appears corrupted/empty, force reload PIXL.json
	...
	if (isThemeCorrupted) {
		...
		if (const_cast<Menu*>(&menu)->LoadThemePreset("PIXL")) {
			logger::info("Successfully recovered with PIXL.json theme");
		} else {
			logger::error("Failed to reload PIXL.json - ImGui may revert to hardcoded defaults");
		}
	}
	...
	// ScaleAllSizes skips border and separator sizes â€” scale them manually, flooring non-zero values at 1px
	...
	// PIXL's player-facing identity is intentionally stable across themes: themes
	// can still control the broad palette, fonts and spacing, while interaction
	// affordances retain the renderer's cyan technical accent.
	const ImVec4 pixlCyan{ 0.20f, 0.83f, 0.86f, 1.00f };
	const ImVec4 pixlCyanHovered{ 0.34f, 0.94f, 0.96f, 1.00f };
	ImVec4 pixlCyanSoft = pixlCyan;
	pixlCyanSoft.w = 0.48f;
	colors[ImGuiCol_CheckMark] = pixlCyanHovered;
	colors[ImGuiCol_SliderGrab] = pixlCyan;
	colors[ImGuiCol_SliderGrabActive] = pixlCyanHovered;
	colors[ImGuiCol_TabHovered] = pixlCyanSoft;
	colors[ImGuiCol_NavHighlight] = pixlCyanSoft;
	colors[ImGuiCol_DragDropTarget] = pixlCyanHovered;

	style.WindowRounding = std::max(style.WindowRounding, Constants::PRODUCT_WINDOW_ROUNDING * scaleFactor);

	// PASS A: the bespoke chrome owns the visible shell. Keep native ImGui
	// geometry restrained underneath it so default rounding/padding does not
	// fight the reference-driven metal framing.
	style.ChildRounding = std::min(style.ChildRounding, 4.0f * scaleFactor);
	style.FrameRounding = std::min(style.FrameRounding, 3.0f * scaleFactor);
	style.TabRounding = std::min(style.TabRounding, 3.0f * scaleFactor);
	style.ItemSpacing.x = std::max(style.ItemSpacing.x, 7.0f * scaleFactor);
	style.ItemSpacing.y = std::max(style.ItemSpacing.y, 6.0f * scaleFactor);
	style.ChildRounding = std::max(style.ChildRounding, Constants::PRODUCT_PANEL_ROUNDING * scaleFactor);
	style.FrameRounding = std::max(style.FrameRounding, Constants::PRODUCT_CONTROL_ROUNDING * scaleFactor);
	style.PopupRounding = std::max(style.PopupRounding, Constants::PRODUCT_PANEL_ROUNDING * scaleFactor);

	// PASS A8: closed-product geometry. Layout/padding is owned by PIXL.
	style.WindowPadding = ImVec2(10.0f * scaleFactor, 10.0f * scaleFactor);
	style.FramePadding = ImVec2(8.0f * scaleFactor, 5.0f * scaleFactor);
	style.ItemSpacing = ImVec2(8.0f * scaleFactor, 6.0f * scaleFactor);
	style.ItemInnerSpacing = ImVec2(6.0f * scaleFactor, 4.0f * scaleFactor);
	style.CellPadding = ImVec2(6.0f * scaleFactor, 4.0f * scaleFactor);
	style.WindowRounding = 3.0f * scaleFactor;
	style.ChildRounding = 2.0f * scaleFactor;
	style.FrameRounding = 2.0f * scaleFactor;
	style.PopupRounding = 2.0f * scaleFactor;
	style.TabRounding = 2.0f * scaleFactor;
	style.ScrollbarRounding = 2.0f * scaleFactor;
	style.GrabRounding = 1.0f * scaleFactor;

	colors[ImGuiCol_WindowBg] = ImVec4(0.027f, 0.035f, 0.043f, 0.98f);
	colors[ImGuiCol_ChildBg] = ImVec4(0.039f, 0.051f, 0.063f, 0.96f);
	colors[ImGuiCol_PopupBg] = ImVec4(0.041f, 0.052f, 0.061f, 0.99f);
	colors[ImGuiCol_FrameBg] = ImVec4(0.063f, 0.078f, 0.086f, 1.00f);
	colors[ImGuiCol_FrameBgHovered] = ImVec4(0.090f, 0.110f, 0.120f, 1.00f);
	colors[ImGuiCol_FrameBgActive] = ImVec4(0.048f, 0.062f, 0.070f, 1.00f);
	colors[ImGuiCol_Button] = ImVec4(0.070f, 0.086f, 0.098f, 1.00f);
	colors[ImGuiCol_ButtonHovered] = ImVec4(0.105f, 0.125f, 0.135f, 1.00f);
	colors[ImGuiCol_ButtonActive] = ImVec4(0.055f, 0.069f, 0.078f, 1.00f);
	colors[ImGuiCol_Header] = ImVec4(0.073f, 0.091f, 0.101f, 0.82f);
	colors[ImGuiCol_HeaderHovered] = ImVec4(0.105f, 0.128f, 0.139f, 0.92f);
	colors[ImGuiCol_HeaderActive] = ImVec4(0.061f, 0.078f, 0.087f, 0.96f);
	colors[ImGuiCol_Border] = ImVec4(0.33f, 0.38f, 0.40f, 0.72f);
	colors[ImGuiCol_Separator] = ImVec4(0.28f, 0.33f, 0.35f, 0.72f);

	// PASS A8.1: scrolling is part of the fixed PIXL product language.
	// Theme/user opacity no longer controls whether the scroll affordance is
	// visible. Keep it narrow, quiet and readable.
	style.ScrollbarSize = 8.0f * scaleFactor;
	style.ScrollbarRounding = 2.0f * scaleFactor;
	colors[ImGuiCol_ScrollbarBg] = ImVec4(0.018f, 0.024f, 0.028f, 0.72f);
	colors[ImGuiCol_ScrollbarGrab] = ImVec4(0.30f, 0.35f, 0.37f, 0.82f);
	colors[ImGuiCol_ScrollbarGrabHovered] = ImVec4(0.40f, 0.47f, 0.49f, 0.92f);
	colors[ImGuiCol_ScrollbarGrabActive] = ImVec4(0.27f, 0.69f, 0.71f, 0.96f);
}

void ThemeManager::ForceApplyDefaultTheme()
{
	// This function applies PIXL.json colors directly to ImGui, bypassing any hardcoded defaults
	...
	if (!themeManager->LoadTheme("PIXL", defaultThemeSettings)) {
		logger::warn("ForceApplyDefaultTheme: Could not load PIXL.json theme");
		return;
	}
	...
	for (int i = 0; i < ImGuiCol_COUNT; i++)
		colors[i] = palette[i];
	logger::info("ForceApplyDefaultTheme: Applied PIXL.json colors directly to ImGui");
}
...
bool ThemeManager::ReloadFont(const Menu& menu, float& cachedFontSize)
{
	...
	// â”€â”€â”€ CJK Font Merging â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
size_t ThemeManager::DiscoverThemes()
{
	...
	// MO2 Overwrite path: <MO2 instance>/overwrite/SKSE/Plugins/PIXLRenderer/Themes
	auto mo2OverwritePath = parentPath / "overwrite" / "SKSE" / "Plugins" / "PIXLRenderer" / "Themes";
...
void ThemeManager::CreateDefaultThemeFiles()
{
	...
	// The standalone package owns PIXL.json. If it is missing, do not synthesize
	// an inherited-looking substitute; retain ImGui's emergency defaults and log it.
	logger::warn("No PIXL theme file was found; no legacy theme was generated");
}
