...
/**
 * @brief Manages hot-swappable theme system for PIXL Renderer menu
 *
 * THEME JSON SCHEMA:
 * ==================
 ...
 * - Role 1 (Heading):    RenderModule section headers
 ...
 * Fonts must exist in Data\SKSE\Plugins\PIXLRenderer\Fonts\
 *
 * BLUR SHADER SYSTEM:
 * ===================
 ...
 *
 * Theme files location: Data\SKSE\Plugins\PIXLRenderer\Themes\
...
public:
	...
	struct Constants
	{
		...
		// PIXL product visual-language tokens. These are deliberately presentation
		// metrics only; renderer/module behavior remains completely independent.
		static constexpr float PRODUCT_WINDOW_ROUNDING = 8.0f;
		static constexpr float PRODUCT_PANEL_ROUNDING = 6.0f;
		static constexpr float PRODUCT_CONTROL_ROUNDING = 4.0f;
		static constexpr float PRODUCT_NAV_HEIGHT = 38.0f;
		static constexpr float PRODUCT_HEADER_HEIGHT = 68.0f;
		static constexpr float PRODUCT_ACCENT_LINE = 2.0f;
		static constexpr float PRODUCT_PANEL_PADDING = 12.0f;
		...
		static constexpr float SCENE_FEATURE_DROPDOWN_RATIO = 0.45f;   // RenderModule dropdown width ratio
