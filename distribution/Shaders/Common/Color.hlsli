...
#define ENABLE_LL SharedData::linearLightCoreSettings.enableLinearLightCore
...
#	else
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.colorGamma) * SharedData::linearLightCoreSettings.vanillaDiffuseColorMult : color;
#	endif
	}

	float3 Light(float3 color, bool isLinear = false)
	{
		color = (ENABLE_LL && !isLinear) ? pow(abs(color), SharedData::linearLightCoreSettings.lightGamma) : color;
#	if defined(MATERIAL_FORGE)
...
#	endif
	...
	float3 DirectionalLight(float3 color, bool isLinear = false)
	{
		return Light(color, isLinear) *
		       ((ENABLE_LL && !isLinear) ? Math::PI * SharedData::linearLightCoreSettings.directionalLightMult : 1.0f);
	}

	float3 PointLight(float3 color, bool isLinear = false)
	{
		return Light(color, isLinear) *
		       ((ENABLE_LL && !isLinear) ? Math::PI * SharedData::linearLightCoreSettings.pointLightMult : 1.0f);
	}
#	if defined(LIGHTING)
	float3 EmitColor(float3 color)
	{
		return ENABLE_LL ? (pow(abs(color / max(emissiveMult, 1e-5)), SharedData::linearLightCoreSettings.emitColorGamma) * emissiveMult * SharedData::linearLightCoreSettings.emitColorMult) : color;
	}
#	endif
	...
	{
#	if defined(MATERIAL_FORGE)
		return ENABLE_LL ? color * SharedData::linearLightCoreSettings.glowmapMult : LinearToSrgb(color);
#	else
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.glowmapGamma) * SharedData::linearLightCoreSettings.glowmapMult : color;
#	endif
	}

	float3 Ambient(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.ambientGamma) * SharedData::linearLightCoreSettings.ambientMult : color;
	}

	float3 Fog(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.fogGamma) : color;
	}

	float FogAlpha(float alpha)
	{
		return ENABLE_LL ? pow(abs(alpha), SharedData::linearLightCoreSettings.fogAlphaGamma) : alpha;
	}

	float3 Effect(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.effectGamma) : color;
	}
	...
	float3 EffectMult(float3 color)
	{
		if (ENABLE_LL) {
#	if defined(MEMBRANE)
			color *= SharedData::linearLightCoreSettings.membraneEffectMult;
#	elif defined(BLOOD)
			color *= SharedData::linearLightCoreSettings.bloodEffectMult;
#	elif defined(PROJECTED_UV)
			color *= SharedData::linearLightCoreSettings.projectedEffectMult;
#	elif defined(DEFERRED)
			color *= SharedData::linearLightCoreSettings.deferredEffectMult;
#	else
			color *= SharedData::linearLightCoreSettings.otherEffectMult;
#	endif
		}
		return color;
	}
	...
	{
		// Particle/effect lighting must track PIXL's scene-light calibration even
		// when the optional full linear-lighting conversion is disabled. Gating this
		// value behind ENABLE_LL left smoke, mist and magic at source brightness in
		// dark scenes while opaque PBR lighting was correctly reduced.
		return SharedData::linearLightCoreSettings.effectLightingMult;
	}

	float EffectAlpha(float alpha)
	{
		return ENABLE_LL ? pow(abs(alpha), SharedData::linearLightCoreSettings.effectAlphaGamma) : alpha;
	}

	float3 Sky(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.skyGamma) : color;
	}

	float3 Water(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.waterGamma) : color;
	}

	float3 LightVolumes(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.vlGamma) : color;
	}

	float3 ColorToLinear(float3 color)
	{
		return ENABLE_LL ? pow(abs(color), SharedData::linearLightCoreSettings.colorGamma) : color;
	}
	...
	{
		return ENABLE_LL ? SharedData::linearLightCoreSettings.vanillaDiffuseColorMult : 1.0f;
	}
#else
