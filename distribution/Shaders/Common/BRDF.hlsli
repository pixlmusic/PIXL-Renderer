...
namespace BRDF
{
	// Exact x^5 polynomial used by Fresnel and diffuse retro-reflection.
	// FXC otherwise lowers pow(x, 5) through logarithm/exponential sequences on
	// several Skyrim permutations, which is slower and less stable near zero.
	...
	{
		float Fc = Pow5(1.0f - saturate(VdotH));
		return Fc + (1 - Fc) * F0;
	}
	...
	{
		const float Fc = Pow5(1.0f - saturate(VdotH));
	...
	/**
	 * Widens perceptual GGX roughness by the screen-space variance of the shading normal.
	 * The fourth-power conversion operates in GGX alpha-squared space and is stable under
	 * minification, unlike applying a linear bias to perceptual roughness.
	 */
	float FilterRoughnessByNormalVariance(float roughness, float3 shadingNormal, float strength, float varianceClamp)
	{
		float3 dNdx = ddx(shadingNormal);
		float3 dNdy = ddy(shadingNormal);
		float variance = 0.5f * max(strength, 0.0f) * (dot(dNdx, dNdx) + dot(dNdy, dNdy));
	 ...
	 * dielectrics and the final clamp prevents pathological amplification at grazing angles.
	 */
	float3 GGXMultiScatterCompensation(float3 F0, float roughness, float NdotV, float strength)
	{
		float2 dfg = EnvBRDF(roughness, saturate(NdotV));
		float singleScatterAlbedo = max(saturate(dfg.x + dfg.y), 0.25f);
		float3 compensation = 1.0f + saturate(F0) * (rcp(singleScatterAlbedo) - 1.0f);
		compensation = min(compensation, float3(2.0f, 2.0f, 2.0f));
		return lerp(float3(1.0f, 1.0f, 1.0f), compensation, saturate(strength));
	}
}
