...
#	if defined(ATMOSPHERE_PIPELINE)
void ApplyReflectionAtmosphere(inout float3 color, float3 positionWS, float4 screenPosition)
{
	float3 fogColor = Color::Fog(AmbientColor.xyz);
	float4 atmosphere = Atmosphere::GetAtmosphereWithoutVolumes(positionWS, FrameBuffer::CameraPosAdjust.xyz, fogColor, float4(screenPosition.xy * FrameBuffer::DynamicResolutionParams2.xy, screenPosition.z, 1));
	color = lerp(color, atmosphere.xyz, atmosphere.w);
}
#	endif
...
#			if defined(CONTACT_SHADOWS)
	dirShadow = lerp(1.0, ContactShadows::GetScreenSpaceShadow(input.Position.xyz, screenUV, screenNoise), 0.8);
#			endif
	...
	float3 diffuseColor = Color::DirectionalLight(SharedData::DirLightColor.xyz / max(llDirLightMult, 1e-5), SharedData::linearLightCoreSettings.isDirLightLinear) * dirShadow * 0.5 * llDirLightMult * Color::VanillaNormalization();

#			if defined(ATMOSPHERE_PIPELINE)
	if (SharedData::atmosphereSettings.enabled) {
		diffuseColor *= Atmosphere::GetSunlightFogAttenuation(input.WorldPosition.xyz, FrameBuffer::CameraPosAdjust.xyz);
	}
#			endif
...
#			if defined(ATMOSPHERE_PIPELINE)
	if (inReflection && SharedData::atmosphereSettings.enabled) {
		ApplyReflectionAtmosphere(psout.Diffuse.xyz, input.WorldPosition.xyz, input.Position);
	}
#			endif
...
#			if defined(ATMOSPHERE_PIPELINE)
	if (inReflection && SharedData::atmosphereSettings.enabled) {
		ApplyReflectionAtmosphere(color, input.WorldPosition.xyz, input.Position);
	}
#			endif
