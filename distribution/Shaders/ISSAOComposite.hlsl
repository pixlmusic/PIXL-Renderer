...
#	if defined(ATMOSPHERE_PIPELINE) && defined(APPLY_FOG)
...
#		if defined(ATMOSPHERE_PIPELINE)
	bool exponentialHeightFogEnabled = SharedData::atmosphereSettings.enabled;
	...
	if (isGeometryDepth || exponentialHeightFogEnabled) {
		...
		if (exponentialHeightFogEnabled && !Atmosphere::ShouldDisableVanillaFog()) {
			// Apply vanilla fog first, then exp fog on top
			composedColor.xyz = lerp(fogSource, fogFade * fogColor, Color::FogAlpha(fogFactor));
			composedColor.xyz = lerp(composedColor.xyz, fogFade * atmosphere.xyz, atmosphere.w);
		} else if (exponentialHeightFogEnabled) {
			// Disable vanilla fog, only apply exp height fog
			composedColor.xyz = lerp(fogSource, fogFade * atmosphere.xyz, atmosphere.w);
		} else {
			composedColor.xyz = lerp(fogSource, fogFade * fogColor, Color::FogAlpha(fogFactor));
		}
	}
